window.APP_DATA = window.APP_DATA || [];
window.APP_DATA.push({
  id: "tehnicka",
  summary: [
    {
      heading: "Zakon o akreditaciji i norme (HZN)",
      points: [
        "Zakon o akreditaciji uređuje osnivanje i djelatnost tijela koje obavlja poslove nacionalne službe za akreditaciju (HAA — javna ustanova koju osniva Vlada RH), područje akreditacije i akreditaciju u vezi s propisima o ocjenjivanju sukladnosti.",
        "U postupku akreditacije utvrđuje se osposobljenost pravnih i fizičkih osoba prema međunarodnim/europskim normama; akreditacija se daje na zahtjev, sklapa se ugovor, dodjeljuje potvrda o akreditaciji — vremenski je ograničena.",
        "Akreditacija = dokaz kompetentnosti (vrh piramide infrastrukture kvalitete)",
        "Certifikacija = potvrda sukladnosti sustava, proizvoda ili osoba s normom (npr. HRN EN ISO 9001). Akreditacijska tijela akreditiraju certifikacijska tijela.",
        "Norma je dokument za opću i višekratnu uporabu, donesen konsenzusom i odobren od priznatog tijela; same po sebi norme NISU obvezne, ali postaju obvezne kad se na njih pozivaju zakoni ili tehnički propisi.",
        "Hrvatske norme izrađuje Hrvatski zavod za norme (HZN) na prijedlog tehničkih odbora; oznaka HRN, preuzete europske HRN EN — ne treba ih nostrificirati.",
        "Četiri načina nastanka hrvatskih normi: prihvaćanje strane norme uz prijevod (pp), u izvorniku s hrvatskim ovitkom (po), objavom obavijesti o prihvaćanju (pr) i izradba izvorne hrvatske norme (izv).",
        "Tehnički propisi razrađuju temeljne zahtjeve za građevinu i tehnička svojstva proizvoda; donosi ih ministar, objavljuju se u NN i OBVEZNI su. Zahtjeve daju izravno ili upućivanjem na normu."
      ]
    },
    {
      heading: "Tehnički propis za građevinske konstrukcije — opće odredbe",
      points: [
        "Primjenjuje se na konstrukcijske i nekonstrukcijske elemente koji moraju biti mehanički otporni i stabilni, na nove i rekonstruirane konstrukcije: betonske, čelične, spregnute, drvene, zidane, geotehničke, potresno otporne, aluminijske i ostale.",
        "Građevinska konstrukcija = skup građevinskih elemenata raspoređenih i povezanih na projektom određen način, na konačnom mjestu u građevini, radi ispunjavanja mehaničke otpornosti i stabilnosti i dijela zahtjeva sigurnosti u slučaju požara.",
        "Predvidiva djelovanja ne smiju prouzročiti: rušenje, neprihvatljive deformacije, oštećenja drugih dijelova/instalacija zbog deformacija, nerazmjerna oštećenja u odnosu na uzrok i vibracije koje ugrožavaju sigurnost ili izazivaju neugodu.",
        "Dodatna zaštita konstrukcije radi otpornosti na požar smatra se sastavnim dijelom tehničkog rješenja konstrukcije.",
        "Program kontrole i osiguranja kvalitete: svojstva bitnih značajki proizvoda, ispitivanja proizvoda izrađenih na gradilištu, ispitivanja prije uporabe i kod pune zaposjednutosti, opis pokusnog rada, učestalost periodičnih pregleda, popis propisa i normi.",
        "Ako u programu kontrole nije drukčije navedeno, potrebna ispitivanja smatraju se kontrolnim ispitivanjima čiju provedbu određuje nadzorni inženjer.",
        "Probno opterećenje obvezno: cestovni/pješački mostovi raspona ≥15 m, željeznički ≥10 m, tribine i javne građevine za ≥1000 osoba, konstrukcije raspona ≥30 m, kranske staze ≥15 m ili kranovi ≥20 t, brane, spremnici ≥1000 m3, spremnici opasnih tvari ≥20 m3."
      ]
    },
    {
      heading: "Racionalna uporaba energije, energetski certifikat",
      points: [
        "Energetsko svojstvo zgrade izražava se pokazateljem korištenja primarne energije u kWh/(m2·a)",
        "Grijana prostorija = projektna temperatura viša od 12 °C",
        "Faktor oblika f0 = A/Ve (manji je bolji).",
        "Sve nove zgrade od 31. 12. 2020. moraju biti zgrade gotovo nulte energije (nZEB); zgrade tijela javne vlasti već nakon 31. 12. 2018.",
        "Propis se ne primjenjuje na privremene zgrade s rokom uporabe do 2 godine, industrijska postrojenja i zgrade niskih energetskih potreba, hale otvorene više od pola radnog vremena, zgrade bez građevinske dozvole i glavnog projekta te obredne/vjerske zgrade.",
        "Energetski certifikat pribavlja se prije uporabne dozvole, promjene vlasništva ili iznajmljivanja; izdaje ga ovlaštena osoba, a ovlaštenje daje Ministarstvo. Od 1. 7. 2013. obvezan za zgrade/cjeline koje se prodaju.",
        "Certifikat nove zgrade: na temelju glavnog projekta, izjave izvođača, vizualnog pregleda i završnog izvješća nadzornog inženjera; sadrži preporuke za korištenje.",
        "Certifikat postojeće zgrade: energetski pregled + prijedlog ekonomski opravdanih mjera.",
        "Energetski razred (A+ do G, 8 razreda) pokazatelj je specifične godišnje potrebne toplinske energije za grijanje Q''H,nd i specifične godišnje primarne energije Eprim za referentne klimatske podatke.",
        "Javno izlaganje certifikata (prva stranica + stranica s mjerama, A3): zgrade javne namjene korisne površine veće od 250 m2.",
        "Energetski pregled prethodi certificiranju",
        "Energetsko certificiranje = skup radnji u svrhu izdavanja certifikata (Pravilnik o energetskom pregledu i certificiranju)."
      ]
    },
    {
      heading: "Zakon o mjeriteljstvu",
      points: [
        "Uređuje djelokrug državnih i ovlaštenih tijela u mjeriteljstvu, sustav mjernih jedinica, etalone, zahtjeve za mjerila u zakonskom mjeriteljstvu, ovjeravanje mjerila, pretpakovine i mjeriteljsku inspekciju.",
        "Zakonsko mjeriteljstvo = radnje iz zakonskih zahtjeva (mjerenja, jedinice, mjerila)",
        "Temeljno mjeriteljstvo = državni etaloni, sljedivost i jedinstvenost mjerenja, razvoj novih metoda.",
        "Poslove obavljaju: Državni zavod za mjeriteljstvo (DZM), ovlaštena tijela (ovjeravanje mjerila kao javna ovlast) i nacionalni umjerni laboratoriji.",
        "Etalon = ostvarenje definicije veličine s iskazanom vrijednošću i mjernom nesigurnošću (npr. metar); državni etalon priznat odlukom mjerodavnog tijela; referentni etalon služi umjeravanju drugih etalona u organizaciji.",
        "Umjeravanje = uspostava odnosa između vrijednosti etalona i pokazivanja mjerila",
        "Ovjeravanje = ispitivanje i označavanje mjerila kojim se potvrđuje sukladnost s propisanim zahtjevima.",
        "Mjeriteljska sljedivost = povezanost rezultata s referencijom neprekinutim lancem umjeravanja; u RH se primjenjuju SI jedinice."
      ]
    },
    {
      heading: "Granična stanja, koeficijenti sigurnosti, eurokodovi",
      points: [
        "GSN = granično opterećenje pri kojem konstrukcija gubi nosivost (sigurnost)",
        "GSU = gubi uporabivost (pukotine, progibi). GSU je često stroži kriterij.",
        "Parcijalni koeficijenti: GSN 1,35 za vlastito i 1,5 za korisno opterećenje (povoljno korisno se ne uzima); GSU 1,0 za oba. Postoje i koeficijenti za materijale i kombinacije djelovanja.",
        "Eurokodovi = usklađene europske norme za proračun konstrukcija; cilj je jedinstven sustav proračuna u Europi; svaka članica izrađuje nacionalne dodatke (karte potresnih zona, vjetar, snijeg).",
        "EC0 osnove, EC1 djelovanja, EC2 beton, EC3 čelik, EC4 spregnute, EC5 drvo, EC6 zidane, EC7 geotehnika, EC8 potres, EC9 aluminij.",
        "Vodovod i kanalizacija nisu određeni ni jednim eurokodom; u izradi je eurokod za staklene konstrukcije (do tada tehnički propis za staklene konstrukcije).",
        "Uz eurokodove smiju se koristiti tehnički propisi i druge norme; strane norme (npr. ruske) mogu se koristiti za specifične dijelove uz dokazivanje.",
        "Toplinsko-izolacijski proizvodi: sustav 3 (općenito), sustav 1 (viši razredi reakcije na požar A1, A2, B, C), sustav 4 (bez ispitivanja reakcije na požar) — HRN EN 13172."
      ]
    },
    {
      heading: "Betonske, zidane i čelične konstrukcije",
      points: [
        "Beton = građevni proizvod od cementa, agregata, vode i dodataka; NIJE u usklađenom području; sustav ocjenjivanja 2+. Betonski čelik i cement 1+; armatura prema tehničkoj specifikaciji.",
        "Projekt betonske konstrukcije: tehnički opis, proračun mehaničke otpornosti i stabilnosti, program kontrole i osiguranja kvalitete; uporabni vijek najmanje 50 godina; otpornost na požar iznimno se ne dokazuje ako posebnim propisom nije određeno vrijeme očuvanja nosivosti.",
        "Sukladnost betona na gradilištu dokazuje se prema normi određenoj programom kontrole i osiguranja kvalitete iz glavnog projekta.",
        "Dostavnica betona: razred tlačne čvrstoće, max. zrno agregata, konzistencija, udio klorida, razred izloženosti, vrsta cementa, količina, podaci o betonari, oznaka sukladnosti »C«.",
        "Zidane konstrukcije: nearmirano, omeđeno, armirano, prednapeto ziđe; projekt sadrži tehnički opis, proračun i program kontrole. Ne primjenjuje se na industrijske dimnjake, potporne zidove, mostove, brane, spremnike, lukove, svodove, kupole.",
        "Omeđeno ziđe: vertikalni i horizontalni serklaži ploštine ≥0,02 m2 s najmanjom izmjerom 150 mm; predgotovljeno ziđe ne može biti nearmirano; kod okvira ispunjenog ziđem sva vertikalna opterećenja preuzima okvir.",
        "Čelične konstrukcije svrstavaju se u razrede izvedbe EXC1–EXC4 prema HRN EN 1090",
        "Drvena konstrukcija = od cjelovitog drva i materijala na osnovi drva."
      ]
    },
    {
      heading: "Prozori i vrata, dimnjaci, završni radovi",
      points: [
        "Prozori i vrata proizvode se u pogonima izvan gradilišta; profili su neharmonizirano, staklo i okovi harmonizirano područje; uobičajeni sustav 3, vrata na evakuacijskim putevima sustav 1.",
        "Projektom se određuju: otpornost na vjetar, vodonepropusnost, propusnost zraka, prolazak topline, zvučna izolacija, otpornost na požar i dim; ako je vijek građevine duži od vijeka prozora — uvjeti zamjene.",
        "Propis se ne primjenjuje na vrata u potpunosti od stakla, rotirajuća vrata, vrata s mehaničkim pogonom, ostakljene krovove, ovješene fasade, garažna, industrijska vrata i vrata za skloništa.",
        "Dimnjak = dio građevnog sklopa, vertikalni kanal postojan na požar čađe za odvod dimnih plinova",
        "Vrste: montažni (na gradilištu od usklađenih proizvoda), predgotovljeni/sistemski (u pogonu) i zidani (od punih opeka).",
        "Tehnički propis za dimnjake ne primjenjuje se na samostojeći dimnjak i dimnjak isključivo u funkciji proizvodnog pogona; pričuvni dimnjak ima svijetli otvor najmanje 200 cm2 (kruta goriva u izvanrednim uvjetima).",
        "Tehničko rješenje dimnjaka obuhvaćeno je arhitektonskim i/ili građevinskim te strojarskim projektom",
        "Proizvodi za dimnjake: glineni/keramički, metalni, betonski, sistemski dimnjaci.",
        "Završni radovi (18): teracerski, fasaderski, gipsarski, keramičarski, ličilački, soboslikarski, tapetarski, kamenorezački, podopolagački, parketarski, izolaterski, bravarski, limarski, stolarski, roletarski, staklorezački, antikorozijska zaštita, spušteni stropovi i lake pregrade."
      ]
    },
    {
      heading: "Pristupačnost (NN 12/23) — brojke",
      points: [
        "Obvezni elementi: za svladavanje visinskih razlika (rampa, stubište, dizalo, vertikalno podizna i koso podizna platforma), za neovisno življenje (ulazni prostor, WC, kuhinja, šalter...) i u javnom prometu (stajalište, parkiralište, semafor, pješački prijelaz, otok, raskrižje).",
        "Rampa: visinska razlika do 120 cm; nagib do 1:20 (5 %), iznimno 1:12 (8,3 %) do 76 cm; širina 120 cm vani / 90 cm unutra; podest 150 cm na svakih 6 m; rukohvati promjera 4 cm na 60 i 90 cm, produženi 30 cm.",
        "Stubište: stuba max 15 cm, gazište min 33 cm, krak 110 cm unutra / 120 cm vani, kontrastni rub 2 cm, središnji rukohvat ako je krak ≥250 cm, taktilno polje upozorenja 40 cm pred prvom i iza posljednje stube.",
        "Dizalo obvezno za razliku veću od 120 cm: kabina 110x140 cm, vrata 90 cm, ploča na 90–120 cm, rukohvat 90 cm, taktilna crta vođenja 40 cm od ulaza.",
        "Platforma: 110x140 cm, stranice do 120 cm, vrata 90 cm.",
        "Ulazni prostor: vrata 110/210 ili 2x90/210 cm, prag do 2 cm, klizna vrata 90/210 s prostorom 150x150, oznaka na staklu >1,5 m2 na 90–160 cm, vjetrobran 240 cm, osvjetljenje 200 lx.",
        "Taktilna površina: reljef do 5 mm, prepoznatljiva stopalom/bijelim štapom, ne zadržava vodu; obvezna pred stubištem, do dizala, WC-a, šaltera, na peronima, semaforima s više od 2 traka, pješačkim prijelazima.",
        "Stambene zgrade s 10 i više stanova: 1 jednostavno prilagodljiv stan na svakih 10 stanova (ulazna vrata 110 cm, ostala 80 cm, hodnik 120 cm, radna ploha do 85 cm); 5 % pristupačnih parkirališnih mjesta, ne manje od 1.",
        "Iznimno odstupanje kod rekonstrukcije u zaštićenoj urbanističkoj cjelini uz suglasnost ministarstva graditeljstva i mišljenje tijela za zaštitu kulturnih dobara."
      ]
    },
    {
      heading: "Tijela treće strane, tržišni nadzor, ceste i karte",
      points: [
        "Usklađeno područje: prijavljeno tijelo (laboratorij, tijelo za certificiranje kontrole tvorničke proizvodnje, tijelo za certificiranje proizvoda) i TAB — tijelo za tehničko ocjenjivanje (donosi EAD i izdaje ETA).",
        "Neusklađeno područje: odobreno tijelo (certifikat o stalnosti svojstava / certifikat KTP prema hrvatskim normama) i hrvatsko tijelo za tehničko ocjenjivanje (HTTO — hrvatski dokument za ocjenjivanje i hrvatska tehnička ocjena uz suglasnost Ministarstva).",
        "Uz proizvod se prilažu izjava o svojstvima i tehničke upute (na hrvatskom, latinicom; sigurnosne obavijesti, čuvanje, transport, ugradnja, rok ugradnje).",
        "Nadzor na tržištu: tržišni inspektori Državnog inspektorata; nadzor uporabe na gradilištu: građevinski i drugi inspektori.",
        "RAPEX = sustav brze razmjene obavijesti o opasnim proizvodima.",
        "Zaštitni pojas javnih cesta od vanjskog ruba zemljišnog pojasa: autoceste i brze ceste 40 m, državne 25 m, županijske 15 m, lokalne 10 m; zemljišni pojas najmanje 1 m; posebne uvjete daju HAC, HC ili ŽUC.",
        "Nerazvrstana cesta = javno dobro u općoj uporabi u vlasništvu JLS (npr. ceste u gradovima s više od 35.000 stanovnika, pristupne ceste, terminali). Javna cesta postaje javno dobro pravomoćnošću uporabne dozvole.",
        "Službene karte DGU: HOK 1:5000 ili 1:10 000, DOF 1:2000/1:5000 i sitnije, DTK 1:25 000 do 1:250 000, PTK 1:300 000 i sitnije."
      ]
    }
  ],
  questions: [
    {
      q: "Što uređuje Zakon o akreditaciji?",
      options: [
        "Postupke ocjenjivanja sukladnosti proizvoda i isprave o sukladnosti",
        "Osnivanje i djelatnost tijela koje obavlja poslove nacionalne službe za akreditaciju, područje akreditacije i akreditaciju u vezi s propisima o ocjenjivanju sukladnosti",
        "Sustav mjernih jedinica i ovjeravanje mjerila",
        "Izradu i objavu hrvatskih normi"
      ],
      correct: 1,
      explanation: "Zakon o akreditaciji uređuje osnivanje i djelatnost akreditacijskog tijela (HAA), određuje područje u kojemu se provodi akreditacija te akreditaciju u vezi s propisima o ocjenjivanju sukladnosti.",
      source: "Zakon o akreditaciji, čl. 1.",
      freq: 2
    },
    {
      q: "Što se utvrđuje u postupku akreditacije?",
      options: [
        "Sukladnost proizvoda s harmoniziranom normom",
        "Ispunjavanje temeljnih zahtjeva za građevinu",
        "Osposobljenost pravnih i fizičkih osoba za obavljanje poslova za koje se daje akreditacija prema međunarodnim i europskim normama",
        "Točnost mjerila u zakonskom mjeriteljstvu"
      ],
      correct: 2,
      explanation: "U postupku akreditacije utvrđuje se osposobljenost pravnih i fizičkih osoba za poslove za koje se daje akreditacija prema odgovarajućim međunarodnim i europskim normama, odnosno istovrijednim nacionalnim normama.",
      source: "Zakon o akreditaciji",
      freq: 1
    },
    {
      q: "Koja je tvrdnja o dodijeljenoj akreditaciji točna?",
      options: [
        "Akreditacija je vremenski ograničena i dodjeljuje se potvrdom o akreditaciji na temelju ugovora",
        "Akreditacija se dodjeljuje trajno, bez vremenskog ograničenja",
        "Akreditaciju dodjeljuje Ministarstvo graditeljstva rješenjem",
        "Akreditacijsko tijelo smije i samo obavljati poslove ocjenjivanja sukladnosti koje akreditira"
      ],
      correct: 0,
      explanation: "Akreditacija se obavlja na zahtjev, akreditacijsko tijelo sklapa ugovor s podnositeljem, a ako utvrdi ispunjavanje zahtjeva normi dodjeljuje potvrdu o akreditaciji koja je vremenski ograničena. Akreditacijsko tijelo ne smije obavljati poslove tijela koja akreditira.",
      source: "Zakon o akreditaciji"
    },
    {
      q: "Koja je razlika između akreditacije i certifikacije?",
      options: [
        "Akreditacija se odnosi na proizvode, a certifikacija na laboratorije",
        "Nema razlike, riječ je o istovjetnim postupcima",
        "Certifikacija dokazuje kompetentnost tijela, a akreditacija sukladnost proizvoda",
        "Akreditacijom se dokazuje kompetentnost za ispitivanja, mjerenja, certifikaciju ili inspekciju, a certifikacijom se potvrđuje sukladnost sustava, proizvoda ili osoba s normom"
      ],
      correct: 3,
      explanation: "Akreditacija je vrh piramide infrastrukture kvalitete: akreditacijska tijela akreditiraju certifikacijska tijela (rezultat: potvrda o akreditaciji), a certifikacijska tijela certificiraju krajnje korisnike (rezultat: certifikat, npr. prema HRN EN ISO 9001).",
      source: "Zakon o akreditaciji",
      freq: 1
    },
    {
      q: "Tko izrađuje hrvatske norme?",
      options: [
        "Hrvatska akreditacijska agencija (HAA)",
        "Ministarstvo nadležno za graditeljstvo",
        "Državni zavod za mjeriteljstvo (DZM)",
        "Hrvatski zavod za norme (HZN) na prijedlog tehničkih odbora"
      ],
      correct: 3,
      explanation: "Norme izrađuje hrvatsko normirno tijelo — Hrvatski zavod za norme (HZN) — na prijedlog tehničkih odbora.",
      source: "Zakon o normizaciji"
    },
    {
      q: "Što od navedenog NIJE cilj normizacije?",
      options: [
        "Otklanjanje tehničkih zapreka u međunarodnoj trgovini",
        "Povećanje raznolikosti proizvoda na tržištu",
        "Osiguranje spojivosti i zamjenjivosti",
        "Povećanje razine sigurnosti proizvoda i procesa"
      ],
      correct: 1,
      explanation: "Ciljevi normizacije su sigurnost, promicanje kakvoće, svrsishodna uporaba rada, materijala i energije, poboljšanje učinkovitosti, OGRANIČENJE raznolikosti, spojivost i zamjenjivost te otklanjanje tehničkih zapreka u trgovini.",
      source: "Zakon o normizaciji",
      freq: 1
    },
    {
      q: "Kako se označavaju preuzete europske norme u Hrvatskoj i treba li ih nostrificirati?",
      options: [
        "HRN EN; ne treba ih nostrificirati",
        "HRN EN; treba ih nostrificirati u HZN-u",
        "EN HR; nostrificira ih Ministarstvo",
        "HRN ISO; nostrifikacija je obvezna za građevne proizvode"
      ],
      correct: 0,
      explanation: "Hrvatske norme označavaju se HRN, a preuzete europske norme HRN EN. Preuzete EU norme ne treba nostrificirati.",
      source: "Zakon o normizaciji"
    },
    {
      q: "Za građevni proizvod koji nije obuhvaćen hrvatskom normom ni usklađenom tehničkom specifikacijom, tko na traženje proizvođača izrađuje hrvatski dokument za ocjenjivanje?",
      options: [
        "Hrvatski zavod za norme uz suglasnost HAA",
        "Prijavljeno tijelo uz suglasnost Europske komisije",
        "Proizvođač sam, uz ovjeru Državnog inspektorata",
        "Hrvatsko tijelo za tehničko ocjenjivanje (HTTO) uz prethodnu suglasnost Ministarstva"
      ],
      correct: 3,
      explanation: "Prema čl. 37. Zakona o građevnim proizvodima, HTTO uz prethodnu suglasnost Ministarstva izrađuje hrvatski dokument za ocjenjivanje i na temelju njega izdaje hrvatsku tehničku ocjenu. U EU sustavu TAB donosi europski dokument za ocjenjivanje i izdaje europsku tehničku ocjenu.",
      source: "Zakon o građevnim proizvodima, čl. 37.",
      freq: 1
    },
    {
      q: "Tko donosi tehničke propise i jesu li obvezni?",
      options: [
        "Hrvatski zavod za norme; obvezni su samo ako to odredi Vlada",
        "Hrvatska komora inženjera građevinarstva; neobvezni su",
        "Hrvatski sabor; obvezni su",
        "Ministar; objavljuju se u Narodnim novinama i obvezni su"
      ],
      correct: 3,
      explanation: "Tehnički propisi razrađuju temeljne zahtjeve za građevinu, tehnička svojstva građevnih proizvoda i druge tehničke zahtjeve. Donosi ih ministar, objavljuju se u NN i obvezni su.",
      source: "Zakon o gradnji; tehnički propisi",
      freq: 1
    },
    {
      q: "Na koji način tehnički propis daje tehničke zahtjeve?",
      options: [
        "Isključivo izravno, bez pozivanja na norme",
        "Isključivo upućivanjem na europske norme",
        "Izravno ili upućivanjem na normu, tehničku specifikaciju ili upute za primjenu, ili uključivanjem sadržaja tih dokumenata",
        "Upućivanjem na projektne zadatke investitora"
      ],
      correct: 2,
      explanation: "Tehnički propis je propis u kojemu se tehnički zahtjevi daju izravno ili upućivanjem na normu, tehničku specifikaciju ili upute za primjenu, ili uključivanjem sadržaja tih dokumenata; time norme postaju obvezne.",
      source: "Zakon o gradnji; tehnički propisi"
    },
    {
      q: "Na koje se konstrukcije primjenjuje Tehnički propis za građevinske konstrukcije?",
      options: [
        "Na nove i rekonstruirane betonske, čelične, spregnute, drvene, zidane, geotehničke, potresno otporne, aluminijske i ostale konstrukcije",
        "Samo na nove betonske i čelične konstrukcije",
        "Samo na konstrukcijske elemente zgrada, ne i na inženjerske građevine",
        "Na sve građevine osim rekonstruiranih"
      ],
      correct: 0,
      explanation: "Propis se primjenjuje na konstrukcijske i nekonstrukcijske elemente koji moraju biti mehanički otporni i stabilni, na nove i rekonstruirane konstrukcije: betonske, čelične, spregnute od čelika i betona, drvene, zidane, geotehničke, potresno otporne, aluminijske te ostale.",
      source: "Tehnički propis za građevinske konstrukcije (NN 17/17, 75/20, 7/22), čl. 2.",
      freq: 2
    },
    {
      q: "Kako Tehnički propis za građevinske konstrukcije definira građevinsku konstrukciju?",
      options: [
        "Skup građevnih proizvoda ugrađenih u građevinu prema izjavi o svojstvima",
        "Skup građevinskih elemenata raspoređenih i povezanih na projektom određen način, na konačnom mjestu u građevini, radi mehaničke otpornosti i stabilnosti i dijela zahtjeva sigurnosti u slučaju požara",
        "Dio građevine koji nosi samo vlastitu težinu",
        "Nosivi sustav zgrade bez temelja"
      ],
      correct: 1,
      explanation: "Građevinska konstrukcija je skup građevinskih elemenata raspoređenih i povezanih na projektom određen način na konačnom mjestu u građevini, čija je svrha ispunjavanje temeljnog zahtjeva mehaničke otpornosti i stabilnosti i dijela zahtjeva sigurnosti u slučaju požara (očuvanje nosivosti).",
      source: "Tehnički propis za građevinske konstrukcije (NN 17/17, 75/20, 7/22)"
    },
    {
      q: "Što od navedenog NIJE među posljedicama koje predvidiva djelovanja prema Tehničkom propisu za građevinske konstrukcije ne smiju prouzročiti?",
      options: [
        "Rušenje cijele građevine ili njezinog dijela",
        "Vibracije koje ugrožavaju sigurnost ili izazivaju neugodu korisnika",
        "Oštećenja nerazmjerna izvornom uzroku",
        "Prekoračenje dopuštene godišnje potrošnje primarne energije"
      ],
      correct: 3,
      explanation: "Tehnička svojstva konstrukcije moraju osigurati da djelovanja ne prouzroče rušenje, neprihvatljive deformacije, oštećenja drugih dijelova/instalacija zbog deformacija, nerazmjerna oštećenja i opasne ili neugodne vibracije. Potrošnja energije predmet je propisa o racionalnoj uporabi energije.",
      source: "Tehnički propis za građevinske konstrukcije (NN 17/17, 75/20, 7/22)"
    },
    {
      q: "Što sadrži program kontrole i osiguranja kvalitete u projektu pojedine struke?",
      options: [
        "Samo popis građevnih proizvoda i njihovih proizvođača",
        "Troškovnik radova i dinamički plan građenja",
        "Svojstva bitnih značajki proizvoda, ispitivanja proizvoda izrađenih na gradilištu, ispitivanja prije uporabe, opis pokusnog rada, učestalost periodičnih pregleda i popis propisa i normi",
        "Geodetski elaborat i iskaz površina"
      ],
      correct: 2,
      explanation: "Program kontrole i osiguranja kvalitete sadrži pregled i specificirana svojstva svih proizvoda, opis potrebnih ispitivanja i zahtijevanih rezultata, uvjete građenja, opis pokusnog rada, zahtjeve periodičnih pregleda te popis propisa i normi.",
      source: "Zakon o gradnji; Tehnički propis za građevinske konstrukcije",
      freq: 2
    },
    {
      q: "Ako u programu kontrole i osiguranja kvalitete nije drukčije navedeno, provedba potrebnih ispitivanja smatra se:",
      options: [
        "Tekućim ispitivanjima proizvođača u kontroli tvorničke proizvodnje",
        "Kontrolnim ispitivanjima odnosno kontrolnim postupcima čiju provedbu određuje nadzorni inženjer",
        "Ispitivanjima koje provodi građevinski inspektor",
        "Dobrovoljnim ispitivanjima izvođača"
      ],
      correct: 1,
      explanation: "Ako u programu kontrole i osiguranja kvalitete nije drukčije navedeno, provedba potrebnih ispitivanja i postupaka dokazivanja smatra se kontrolnim ispitivanjima čiju provedbu određuje nadzorni inženjer.",
      source: "Tehnički propis za građevinske konstrukcije",
      freq: 1
    },
    {
      q: "Koje su dvije grane mjeriteljstva prema Zakonu o mjeriteljstvu?",
      options: [
        "Industrijsko i znanstveno mjeriteljstvo",
        "Državno i privatno mjeriteljstvo",
        "Zakonsko i temeljno mjeriteljstvo",
        "Primarno i sekundarno mjeriteljstvo"
      ],
      correct: 2,
      explanation: "Zakonsko mjeriteljstvo odnosi se na radnje koje nastaju iz zakonskih zahtjeva (mjerenja, mjerne jedinice, mjerila, metode), a temeljno mjeriteljstvo istražuje stalnice prirodnih pojava — uspostava državnih etalona, sljedivosti i razvoj novih mjernih metoda.",
      source: "Zakon o mjeriteljstvu",
      freq: 1
    },
    {
      q: "Tko obavlja poslove mjeriteljstva u Republici Hrvatskoj?",
      options: [
        "Hrvatska akreditacijska agencija i Hrvatski zavod za norme",
        "Isključivo Državni zavod za mjeriteljstvo",
        "Ministarstvo gospodarstva i Državni inspektorat",
        "Državni zavod za mjeriteljstvo, ovlaštena tijela i pravne osobe koje obavljaju poslove nacionalnog umjernog laboratorija"
      ],
      correct: 3,
      explanation: "Poslove mjeriteljstva obavljaju DZM (upravne stvari, ovjeravanje, službena mjerenja, nadzor), ovlaštena tijela (ovjeravanje zakonitih mjerila kao javna ovlast) i nacionalni umjerni laboratoriji.",
      source: "Zakon o mjeriteljstvu",
      freq: 2
    },
    {
      q: "Što je mjerni etalon?",
      options: [
        "Uređaj koji se upotrebljava za mjerenja u prometu robe",
        "Kombinacija proizvoda i ambalaže u koju je zapakiran",
        "Postupak ispitivanja i označavanja mjerila",
        "Ostvarenje definicije dane veličine s iskazanom vrijednošću i mjernom nesigurnošću koje se upotrebljava kao referencija"
      ],
      correct: 3,
      explanation: "Etalon je ostvarenje definicije veličine s iskazanom vrijednošću i mjernom nesigurnošću koje služi kao referencija (npr. etalon za duljinu je metar). Državni etalon priznat je odlukom mjerodavnog tijela; referentni etalon služi umjeravanju drugih etalona u organizaciji.",
      source: "Zakon o mjeriteljstvu"
    },
    {
      q: "Koja je razlika između umjeravanja i ovjeravanja mjerila?",
      options: [
        "Umjeravanje uspostavlja odnos između vrijednosti etalona i pokazivanja mjerila, a ovjeravanje je ispitivanje i označavanje mjerila kojim se potvrđuje sukladnost s propisanim zahtjevima",
        "Ovjeravanje provodi proizvođač, a umjeravanje korisnik",
        "Umjeravanje je zakonski obvezno, a ovjeravanje dobrovoljno",
        "Nema razlike, riječ je o sinonimima"
      ],
      correct: 0,
      explanation: "Umjeravanje pod određenim uvjetima uspostavlja odnos između vrijednosti veličine etalona i pokazivanja mjerila. Ovjeravanje obuhvaća ispitivanje i označavanje mjerila, čime se potvrđuje da je mjerilo usklađeno s propisanim mjeriteljskim zahtjevima.",
      source: "Zakon o mjeriteljstvu"
    },
    {
      q: "Koji je sustav ocjenjivanja i provjere stalnosti svojstava propisan za toplinsko-izolacijske proizvode koji podliježu propisima zaštite od požara (razredi A1, A2, B, C)?",
      options: [
        "Sustav 4",
        "Sustav 3",
        "Sustav 2+",
        "Sustav 1"
      ],
      correct: 3,
      explanation: "Prema HRN EN 13172 sustav 3 propisan je za sve primjene (svojstva osim požara), sustav 1 za proizvode s jasno odredivom mjerom u proizvodnji koja ih svrstava u viši razred reakcije na požar (A1, A2, B, C), a sustav 4 za proizvode kod kojih nije potrebno ispitivanje reakcije na požar.",
      source: "HRN EN 13172"
    },
    {
      q: "Što označava granično stanje uporabivosti (GSU)?",
      options: [
        "Granično opterećenje pri kojem konstrukcija gubi uporabivost (pukotine, progibi) iako nije ugrožena sigurnost",
        "Opterećenje pri kojem dolazi do loma materijala",
        "Granično opterećenje pri kojem konstrukcija gubi nosivost",
        "Opterećenje koje uzrokuje rušenje temelja"
      ],
      correct: 0,
      explanation: "GSN označava granično opterećenje pri kojem konstrukcija gubi nosivost (sigurnost), a GSU opterećenje pri kojem gubi uporabivost — pojava pukotina i progiba. Češće je GSU stroži kriterij pa se prema njemu projektira.",
      source: "HRN EN 1990 (Eurokod 0)"
    },
    {
      q: "Koliki su parcijalni koeficijenti sigurnosti za djelovanja kod graničnog stanja nosivosti?",
      options: [
        "1,35 za vlastito i 1,5 za korisno opterećenje",
        "1,0 za vlastito i 1,0 za korisno opterećenje",
        "1,5 za vlastito i 1,35 za korisno opterećenje",
        "1,15 za vlastito i 1,5 za korisno opterećenje"
      ],
      correct: 0,
      explanation: "GSN: 1,35 za vlastito (stalno) i 1,5 za korisno (promjenjivo) opterećenje; ako korisno djeluje povoljno (npr. protiv odižućeg vjetra) ne uzima se u obzir. GSU: 1,0 za oba.",
      source: "HRN EN 1990 (Eurokod 0)"
    },
    {
      q: "Što su eurokodovi i što je svaka država članica obvezna izraditi?",
      options: [
        "Usklađene europske norme za proračun konstrukcija; svaka članica izrađuje nacionalne dodatke (potresne zone, vjetar, snijeg)",
        "Europske uredbe o građevnim proizvodima; svaka članica izrađuje tehničke propise",
        "Preporuke CEN-a bez obveze primjene; nacionalni dodaci nisu potrebni",
        "Norme za ocjenjivanje sukladnosti proizvoda; članice imenuju TAB-ove"
      ],
      correct: 0,
      explanation: "Eurokodovi su set usklađenih europskih normi za statički proračun konstrukcija s ciljem jedinstvenog sustava proračuna u Europi. Svaka članica obvezno ih prihvaća i izrađuje nacionalne dodatke koji uređuju specifičnosti (karte potresnih zona, opterećenje vjetrom i snijegom).",
      source: "HRN EN 1990–1999 (Eurokodovi)",
      freq: 2
    },
    {
      q: "Koji eurokod uređuje geotehničko projektiranje?",
      options: [
        "EC5",
        "EC6",
        "EC7",
        "EC9"
      ],
      correct: 2,
      explanation: "EC7 je geotehničko projektiranje. EC5 su drvene konstrukcije, EC6 zidane, EC9 aluminijske; EC4 spregnute čelično-betonske, EC8 potresna otpornost.",
      source: "HRN EN 1997 (Eurokod 7)"
    },
    {
      q: "Koji eurokod određuje projektiranje vodovoda i kanalizacije?",
      options: [
        "Eurokod 1 (djelovanja na konstrukcije)",
        "Eurokod 7 (geotehničko projektiranje)",
        "Eurokod 2 (betonske konstrukcije)",
        "Nijedan — projektiranje vodovoda i kanalizacije određeno je drugim normama"
      ],
      correct: 3,
      explanation: "Projektiranje vodovoda i kanalizacije nije određeno ni jednim eurokodom, već drugim normama.",
      source: "Eurokodovi"
    },
    {
      q: "Za koje je konstrukcije eurokod još u izradi i što se koristi u međuvremenu?",
      options: [
        "Za aluminijske konstrukcije; koristi se EC3",
        "Za staklene konstrukcije; koristi se tehnički propis za staklene konstrukcije",
        "Za zidane konstrukcije; koriste se njemačke DIN norme",
        "Za geotehničke konstrukcije; koriste se ruske norme"
      ],
      correct: 1,
      explanation: "U izradi je eurokod za staklene konstrukcije; dok se ne donese, koristi se tehnički propis za staklene konstrukcije. Strane norme (npr. ruske) mogu se iznimno koristiti za specifične dijelove, ali se moraju dokazati.",
      source: "Eurokodovi",
      freq: 1
    },
    {
      q: "Što je beton prema Tehničkom propisu za građevinske konstrukcije?",
      options: [
        "Građevni proizvod koji se sastoji od cementa, agregata, vode i dodataka betonu",
        "Građevni proizvod koji se sastoji od cementa, pijeska i vode",
        "Predgotovljeni element od armiranog cementnog morta",
        "Mješavina veziva i agregata bez dodataka"
      ],
      correct: 0,
      explanation: "Beton je građevni proizvod koji se sastoji od cementa, agregata, vode i dodataka betonu. Betonska konstrukcija je konstrukcija od nearmiranog, armiranog ili prednapetog betona.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za betonske konstrukcije",
      freq: 1
    },
    {
      q: "Je li beton u usklađenom (harmoniziranom) području i koji je sustav ocjenjivanja i provjere stalnosti svojstava za beton?",
      options: [
        "Da; sustav 1+",
        "Da; sustav 3",
        "Ne; sustav 2+",
        "Ne; sustav 4"
      ],
      correct: 2,
      explanation: "Beton nije u usklađenom području; sustav ocjenjivanja i provjere stalnosti svojstava za beton je 2+. Betonski čelik i cement su u sustavu 1+, a armatura prema odredbama tehničke specifikacije.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za betonske konstrukcije",
      freq: 1
    },
    {
      q: "Što od navedenog sadrži dostavnica betona?",
      options: [
        "Razred tlačne čvrstoće, maksimalno zrno agregata, konzistenciju, udio klorida, razred izloženosti, vrstu cementa, količinu, podatke o betonari i oznaku sukladnosti »C«",
        "Oznaku CE, izjavu o svojstvima i tehničku uputu",
        "Broj građevinske dozvole, ime nadzornog inženjera i vrijeme ugradnje",
        "Statički proračun elementa u koji se beton ugrađuje"
      ],
      correct: 0,
      explanation: "Dostavnica sadrži podatke o betonu (razred tlačne čvrstoće, max. zrno agregata, konzistenciju npr. slijeganjem, udio klorida, razred izloženosti, vrstu cementa), količinu, podatke o betonari i oznaku sukladnosti »C« (neusklađeno područje).",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za betonske konstrukcije"
    },
    {
      q: "U kojem slučaju se prema posebnim pravilima za betonske konstrukcije iznimno ne mora dokazivati otpornost na požar?",
      options: [
        "Kad je građevina niža od 3 etaže",
        "Kad je uporabni vijek građevine kraći od 50 godina",
        "Kad je beton u sustavu 2+",
        "Kad posebnim propisom nije određeno vrijeme očuvanja nosivosti betonske konstrukcije u slučaju požara za tu građevinu"
      ],
      correct: 3,
      explanation: "Mehanička otpornost i stabilnost te otpornost na požar dokazuju se proračunima u glavnom projektu; iznimno, otpornost na požar ne mora se dokazivati ako posebnim propisom nije određeno vrijeme očuvanja nosivosti u požaru. Uporabni vijek je najmanje 50 godina.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za betonske konstrukcije",
      freq: 1
    },
    {
      q: "Koje su vrste ziđa od kojih se izvodi zidana konstrukcija?",
      options: [
        "Puno, šuplje i lagano ziđe",
        "Jednostruko, dvostruko i troslojno ziđe",
        "Opečno, kameno i betonsko ziđe",
        "Nearmirano, omeđeno, armirano i prednapeto ziđe"
      ],
      correct: 3,
      explanation: "Zidana konstrukcija izvodi se od nearmiranog, omeđenog, armiranog i prednapetog ziđa. Po ulozi ziđe može biti nosivo, fasadno, obložno, pregradno, vezno, ispunsko i protupožarno.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za zidane konstrukcije",
      freq: 1
    },
    {
      q: "Na što se NE primjenjuju posebna pravila za zidane konstrukcije?",
      options: [
        "Na pregradno ziđe i parapete koji nisu dio zidane konstrukcije",
        "Na protupožarno ziđe",
        "Na fasadno i obložno ziđe",
        "Na industrijske dimnjake, potporne zidove, mostove, brane, spremnike za tekućine te lukove, svodove i kupole"
      ],
      correct: 3,
      explanation: "Pravila se primjenjuju na nosivo, fasadno, obložno, pregradno, vezno, ispunsko i protupožarno ziđe, ali ne na zidane konstrukcije industrijskih dimnjaka, potpornih zidova, propusta, mostova, brana, spremnika, konstrukcija s pretežito dinamičkim djelovanjem te lukova, svodova i kupola.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za zidane konstrukcije",
      freq: 1
    },
    {
      q: "Koje su najmanje dimenzije omeđujućih vijenaca (serklaža) kod omeđenog ziđa?",
      options: [
        "Ploština presjeka 0,04 m2 s najmanjom izmjerom 200 mm",
        "Ploština presjeka 0,01 m2 s najmanjom izmjerom 100 mm",
        "Ploština presjeka 0,02 m2 s najmanjom izmjerom 150 mm u tlocrtu zida",
        "Ploština presjeka 0,03 m2 s najmanjom izmjerom 250 mm"
      ],
      correct: 2,
      explanation: "Omeđeno ziđe mora imati vertikalne i horizontalne AB ili armirane zidane serklaže ploštine presjeka ne manje od 0,02 m2 s najmanjom izmjerom 150 mm u tlocrtu zida. Serklaži nemaju učinak okvira, već sa ziđem daju nosivost i duktilnost.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za zidane konstrukcije"
    },
    {
      q: "Koja je tvrdnja o okviru ispunjenom ziđem točna?",
      options: [
        "Ziđe ispune preuzima sva vertikalna opterećenja gornjih katova",
        "Sva vertikalna opterećenja preuzima AB ili čelični okvir, a ziđe ispune nosi samo vlastitu težinu i povećava krutost na horizontalno opterećenje",
        "Okvir i ziđe podjednako preuzimaju vertikalna opterećenja",
        "Ziđe ispune ne utječe na horizontalnu krutost"
      ],
      correct: 1,
      explanation: "Kod okvira ispunjenog ziđem sva vertikalna opterećenja preuzima okvir, dok ziđe ispune nosi samo vlastitu težinu, ali povećava krutost i nosivost na horizontalno opterećenje. Kod omeđenog ziđa, nasuprot tome, ziđe preuzima vertikalna opterećenja.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za zidane konstrukcije"
    },
    {
      q: "U koje se razrede izvedbe svrstavaju čelične konstrukcije?",
      options: [
        "EXC1 do EXC4 prema HRN EN 1090",
        "A do D prema HRN EN 1993",
        "K1 do K3 prema HRN EN 10025",
        "S235 do S460 prema HRN EN 1090"
      ],
      correct: 0,
      explanation: "Čelična konstrukcija se ovisno o zahtjevima izvedbe svrstava u jedan od razreda izvedbe EXC1, EXC2, EXC3 ili EXC4, sukladno tehničkoj specifikaciji i normi HRN EN 1090.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za čelične konstrukcije"
    },
    {
      q: "Koji je uobičajeni sustav ocjenjivanja za prozore i vrata vanjske ovojnice, a koji za vrata na evakuacijskim putevima?",
      options: [
        "Sustav 1 za ovojnicu, sustav 3 za evakuacijska vrata",
        "Sustav 4 za ovojnicu, sustav 2+ za evakuacijska vrata",
        "Sustav 2+ za oba",
        "Sustav 3 za ovojnicu, sustav 1 za evakuacijska vrata"
      ],
      correct: 3,
      explanation: "Uobičajeni sustav za prozore i vrata vanjske ovojnice zgrada je sustav 3, a za vrata na evakuacijskim putevima sustav 1. Profili su neharmonizirano područje, a staklo i okovi harmonizirano.",
      source: "Tehnički propis za prozore i vrata",
      freq: 2
    },
    {
      q: "Što od navedenog NIJE svojstvo prozora i vrata koje se prema Tehničkom propisu za prozore i vrata određuje projektom građevine?",
      options: [
        "Otpornost na opterećenje vjetrom",
        "Vodonepropusnost pri djelovanju kiše i vjetra",
        "Boja i vrsta drva profila",
        "Zvučna izolacija i otpornost na požar i propuštanje dima"
      ],
      correct: 2,
      explanation: "Projektom se određuju: otpornost na vjetar, vodonepropusnost, propusnost zraka, prolazak topline, zvučna izolacija te otpornost na požar i propuštanje dima (između požarnih sektora). Prozori i vrata proizvode se u pogonima izvan gradilišta.",
      source: "Tehnički propis za prozore i vrata",
      freq: 2
    },
    {
      q: "Na što se NE primjenjuje Tehnički propis za prozore i vrata?",
      options: [
        "Na drvene prozore stambenih zgrada",
        "Na vrata između požarnih sektora",
        "Na vrata i stijene u potpunosti od stakla, rotirajuća vrata, vrata s mehaničkim pogonom, ovješene fasade, garažna i industrijska vrata te vrata za skloništa",
        "Na ulazna vrata javnih zgrada"
      ],
      correct: 2,
      explanation: "Propis se ne primjenjuje na vrata i stijene izrađene u potpunosti od stakla, rotirajuća vrata, vrata s mehaničkim pogonom, ostakljene krovne konstrukcije, ovješene fasade, garažna, industrijska vrata i vrata za skloništa.",
      source: "Tehnički propis za prozore i vrata"
    },
    {
      q: "Na koje se dimnjake NE primjenjuje Tehnički propis za dimnjake u građevinama?",
      options: [
        "Na zidane dimnjake od punih opeka",
        "Na pričuvne dimnjake",
        "Na montažne dimnjake",
        "Na samostojeći dimnjak i dimnjak koji je isključivo u funkciji proizvodnog pogona"
      ],
      correct: 3,
      explanation: "Propis se ne primjenjuje na samostojeći dimnjak i dimnjak koji je isključivo u funkciji proizvodnog pogona. Primjenjuje se na montažne, predgotovljene (sistemske) i zidane dimnjake kao dio građevnog sklopa građevine.",
      source: "Tehnički propis za dimnjake u građevinama",
      freq: 2
    },
    {
      q: "Koje su vrste dimnjaka prema Tehničkom propisu za dimnjake?",
      options: [
        "Montažni, predgotovljeni (sistemski) i zidani",
        "Keramički, metalni i betonski",
        "Samostojeći, prislonjeni i ugrađeni",
        "Jednostruki, dvostruki i višestruki"
      ],
      correct: 0,
      explanation: "Montažni dimnjak izvodi se na gradilištu od usklađenih proizvoda, predgotovljeni (sistemski) sastavlja se u pogonu izvan gradilišta i ugrađuje kao predgotovljeni proizvod, a zidani se zida na gradilištu od punih opeka. Keramički, metalni i betonski su proizvodi za dimnjake.",
      source: "Tehnički propis za dimnjake u građevinama",
      freq: 2
    },
    {
      q: "Što je pričuvni dimnjak?",
      options: [
        "Dimnjak za odvod plinova iz kotlovnice u slučaju kvara glavnog dimnjaka",
        "Dimnjak svijetlog otvora najmanje 100 cm2 za plinska ložišta",
        "Dimnjak najmanje ploštine svijetlog otvora 200 cm2 za priključenje uređaja za loženje na kruta goriva u izvanrednim uvjetima",
        "Dimovod izveden u proizvodnom pogonu"
      ],
      correct: 2,
      explanation: "Pričuvni dimnjak jest dimnjak najmanje ploštine svijetlog otvora 200 cm2 koji služi za priključenje uređaja za loženje na kruta goriva u izvanrednim uvjetima. Tehničko rješenje dimnjaka obuhvaćeno je arhitektonskim/građevinskim i strojarskim projektom.",
      source: "Tehnički propis za dimnjake u građevinama"
    },
    {
      q: "Koji od navedenih radova NIJE završni rad?",
      options: [
        "Teracerski radovi",
        "Armirački radovi",
        "Roletarski radovi",
        "Kamenorezački radovi"
      ],
      correct: 1,
      explanation: "Završni radovi su teracerski, fasaderski, gipsarski, keramičarski, ličilački, soboslikarski, tapetarski, kamenorezački, podopolagački, parketarski, izolaterski, bravarski, limarski, stolarski, roletarski, staklorezački, antikorozijska zaštita te spušteni stropovi i lake pregrade. Armirački radovi su konstrukcijski (grubi) radovi.",
      source: "Vrste završnih radova (skripta)",
      freq: 2
    },
    {
      q: "Kojim se pokazateljem izražava energetsko svojstvo zgrade?",
      options: [
        "Koeficijentom prolaska topline U u W/(m2·K)",
        "Brojčanim pokazateljem korištenja primarne energije u kWh/(m2·a)",
        "Godišnjom emisijom CO2 u t/a",
        "Brojem izmjena zraka n u h-1"
      ],
      correct: 1,
      explanation: "Energetsko svojstvo zgrade je izračunata ili izmjerena količina energije potrebna za karakterističnu uporabu (grijanje, hlađenje, ventilacija, PTV, rasvjeta), a izražava se pokazateljem korištenja primarne energije u kWh/(m2·a).",
      source: "Tehnički propis o racionalnoj uporabi energije i toplinskoj zaštiti u zgradama (NN 128/15+)"
    },
    {
      q: "Što je faktor oblika zgrade f0 i kakve su zgrade povoljnije za fiziku zgrade?",
      options: [
        "Količnik oplošja i obujma grijanog dijela zgrade (A/Ve); povoljnije su nerazvedene zgrade s manjim faktorom oblika",
        "Količnik obujma i oplošja (Ve/A); povoljnije su razvedene zgrade",
        "Omjer površine prozora i pročelja; povoljnije su zgrade s više prozora",
        "Omjer korisne i bruto površine; povoljnije su zgrade s većim faktorom"
      ],
      correct: 0,
      explanation: "Faktor oblika f0 = A/Ve (m-1) je količnik oplošja i obujma grijanog dijela zgrade. Što je oplošje manje, manji je faktor oblika, pa su nerazvedene zgrade bolje jer imaju manju površinu pročelja koja generira toplinske gubitke.",
      source: "Tehnički propis o racionalnoj uporabi energije i toplinskoj zaštiti u zgradama (NN 128/15+)"
    },
    {
      q: "Od kojeg datuma sve nove zgrade moraju biti zgrade gotovo nulte energije?",
      options: [
        "Od 1. srpnja 2013.",
        "Od 31. prosinca 2018.",
        "Od 31. prosinca 2020.",
        "Od 1. siječnja 2025."
      ],
      correct: 2,
      explanation: "Od 31. prosinca 2020. sve nove zgrade moraju biti zgrade gotovo nulte energije, a nove zgrade koje koriste ili su u vlasništvu tijela javne vlasti već nakon 31. prosinca 2018.",
      source: "Tehnički propis o racionalnoj uporabi energije i toplinskoj zaštiti u zgradama (NN 128/15+)"
    },
    {
      q: "Na koje se zgrade NE primjenjuje Tehnički propis o racionalnoj uporabi energije i toplinskoj zaštiti u zgradama?",
      options: [
        "Na stambene zgrade s manje od 10 stanova",
        "Na privremene zgrade s rokom uporabe do 2 godine, zgrade s niskim energetskim potrebama, hale otvorene više od polovice radnog vremena, zgrade bez građevinske dozvole i glavnog projekta te obredne i vjerske zgrade",
        "Na sve nestambene zgrade",
        "Na zgrade grijane iznad 12 °C"
      ],
      correct: 1,
      explanation: "Propis se ne primjenjuje na privremene zgrade do 2 godine, industrijska postrojenja i zgrade niskih energetskih potreba (staklenici, staje), hale koje se drže otvorenima više od pola radnog vremena bez zračnih zavjesa, zgrade iz Pravilnika o jednostavnim građevinama te zgrade u obredne i vjerske svrhe.",
      source: "Tehnički propis o racionalnoj uporabi energije i toplinskoj zaštiti u zgradama (NN 128/15+)"
    },
    {
      q: "Kada se mora pribaviti energetski certifikat i tko izdaje ovlaštenje za njegovo izdavanje?",
      options: [
        "Prije izdavanja građevinske dozvole; ovlaštenje izdaje HKIG",
        "Prije uporabne dozvole, promjene vlasništva ili iznajmljivanja zgrade; ovlaštenje izdaje Ministarstvo",
        "Nakon tehničkog pregleda; ovlaštenje izdaje Državni inspektorat",
        "Samo pri prodaji zgrade; ovlaštenje izdaje jedinica lokalne samouprave"
      ],
      correct: 1,
      explanation: "Prije izdavanja uporabne dozvole odnosno prije promjene vlasništva ili iznajmljivanja zgrade ili njezina dijela mora se pribaviti energetski certifikat koji izdaje ovlaštena osoba; ovlaštenje izdaje Ministarstvo. Kupac/unajmljivač ima pravo uvida prije sklapanja ugovora.",
      source: "Tehnički propis o racionalnoj uporabi energije i toplinskoj zaštiti u zgradama (NN 128/15+)"
    },
    {
      q: "Na temelju čega se izdaje energetski certifikat nove zgrade?",
      options: [
        "Na temelju energetskog pregleda s mjerenjem potrošnje tijekom prve godine uporabe",
        "Na temelju izjave investitora i računa za energente",
        "Na temelju podataka iz glavnog projekta, pisane izjave izvođača, vizualnog pregleda i završnog izvješća nadzornog inženjera",
        "Na temelju termografskog snimanja pročelja"
      ],
      correct: 2,
      explanation: "Certifikat nove zgrade izdaje se na temelju glavnog projekta u dijelu racionalne uporabe energije, pisane izjave izvođača o izvedenim radovima, vizualnog pregleda zgrade i završnog izvješća nadzornog inženjera. Nova zgrada gradi se na temelju akta izdanog nakon 1. 10. 2007.",
      source: "Pravilnik o energetskom pregledu zgrade i energetskom certificiranju (NN 88/17+)"
    },
    {
      q: "U čemu se razlikuje energetski certifikat postojeće zgrade od certifikata nove zgrade?",
      options: [
        "Certifikat postojeće zgrade nema energetski razred",
        "Certifikat nove zgrade vrijedi trajno, a postojeće 5 godina",
        "Certifikat postojeće zgrade izdaje Ministarstvo, a nove ovlaštena osoba",
        "Certifikat postojeće zgrade sadrži prijedlog ekonomski opravdanih mjera za poboljšanje energetskih svojstava, a nove zgrade preporuke za korištenje zgrade"
      ],
      correct: 3,
      explanation: "Za postojeće zgrade certifikat obvezno sadrži prijedlog troškovno učinkovitih mjera poboljšanja temeljen na energetskom pregledu (ušteda, povrat investicije). Za nove zgrade sadrži preporuke za korištenje zgrade radi ispunjenja zahtjeva gospodarenja energijom i očuvanja topline.",
      source: "Pravilnik o energetskom pregledu zgrade i energetskom certificiranju (NN 88/17+)",
      freq: 1
    },
    {
      q: "Što od navedenog NIJE sadržaj energetskog certifikata?",
      options: [
        "Energetski razred i rok važenja certifikata",
        "Podaci o termotehničkim sustavima i korištenju obnovljivih izvora",
        "Troškovnik radova energetske obnove s jediničnim cijenama",
        "Prijedlog mjera i podaci o osobi koja je izradila certifikat"
      ],
      correct: 2,
      explanation: "Certifikat sadrži opće podatke o zgradi, energetski razred, rok važenja, podatke o izdavatelju i sudionicima, oznaku certifikata, podatke o termotehničkim sustavima, energetske potrebe, obnovljive izvore, prijedlog mjera i objašnjenja. Troškovnik nije dio certifikata.",
      source: "Pravilnik o energetskom pregledu zgrade i energetskom certificiranju (NN 88/17+)",
      freq: 1
    },
    {
      q: "Koje zgrade moraju imati javno izložen energetski certifikat?",
      options: [
        "Sve stambene zgrade s 10 i više stanova",
        "Zgrade javne namjene korisne površine veće od 1000 m2",
        "Sve zgrade koje se prodaju ili iznajmljuju",
        "Zgrade javne namjene korisne površine veće od 250 m2"
      ],
      correct: 3,
      explanation: "Zgrade javne namjene ili samostalne uporabne cjeline javne namjene u zgradama mješovite namjene korisne površine veće od 250 m2 moraju imati izdan i javno izložen certifikat (prva stranica i stranica s mjerama, format A3). Od 1. 7. 2013. certifikat moraju imati zgrade/cjeline koje se prodaju.",
      source: "Pravilnik o energetskom pregledu zgrade i energetskom certificiranju (NN 88/17+)",
      freq: 1
    },
    {
      q: "Pokazatelj čega je energetski razred zgrade prikazan u certifikatu?",
      options: [
        "Specifične godišnje potrebne toplinske energije za grijanje Q''H,nd i specifične godišnje primarne energije Eprim za referentne klimatske podatke",
        "Koeficijenta prolaska topline vanjskog zida i prozora",
        "Izmjerene godišnje potrošnje električne energije korisnika",
        "Udjela obnovljivih izvora u isporučenoj energiji"
      ],
      correct: 0,
      explanation: "Energetski razred pokazatelj je specifične godišnje potrebne toplinske energije za grijanje (Q''H,nd) i specifične godišnje primarne energije (Eprim) za referentne klimatske podatke i Algoritmom propisan režim korištenja. Zgrade se svrstavaju u 8 razreda od A+ do G.",
      source: "Pravilnik o energetskom pregledu zgrade i energetskom certificiranju (NN 88/17+)",
      freq: 1
    },
    {
      q: "Za svladavanje kolike visinske razlike se koristi rampa kao element pristupačnosti i koji joj je dopušteni nagib?",
      options: [
        "Do 76 cm; nagib do 1:10",
        "Do 120 cm; nagib do 1:20 (5 %), iznimno 1:12 (8,3 %) za razlike do 76 cm",
        "Do 150 cm; nagib do 1:12",
        "Do 120 cm; nagib do 1:8 (12,5 %)"
      ],
      correct: 1,
      explanation: "Rampa se koristi za visinske razlike do uključivo 120 cm (iznimno vani i više). Nagib do 1:20 (5 %), iznimno 1:12 (8,3 %) za razlike do 76 cm; širina 120 cm vani / 90 cm unutra; podest 150 cm na svakih 6 m; rukohvati promjera 4 cm na 60 i 90 cm.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)",
      freq: 1
    },
    {
      q: "Koje su najveće dopuštene dimenzije stube i najmanja širina stubišnog kraka kod pristupačnog stubišta?",
      options: [
        "Visina stube do 17 cm, gazište 30 cm, krak 100 cm",
        "Visina stube do 15 cm, gazište najmanje 33 cm, krak najmanje 110 cm unutra i 120 cm vani",
        "Visina stube do 18 cm, gazište 28 cm, krak 90 cm",
        "Visina stube do 15 cm, gazište 30 cm, krak 150 cm"
      ],
      correct: 1,
      explanation: "Stubište: visina stube najviše 15 cm, širina nastupne plohe najmanje 33 cm, svijetla širina kraka 110 cm unutra i 120 cm vani, kontrastni rub 2 cm, središnji rukohvat ako je krak ≥250 cm, taktilno polje upozorenja širine 40 cm pred prvom i iza posljednje stube.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)",
      freq: 1
    },
    {
      q: "Koje su najmanje unutarnje dimenzije pristupačnog dizala i širina vrata?",
      options: [
        "100 x 120 cm, vrata 80 cm",
        "120 x 150 cm, vrata 100 cm",
        "110 x 140 cm, vrata 90 cm",
        "90 x 130 cm, vrata 90 cm"
      ],
      correct: 2,
      explanation: "Dizalo se obvezno primjenjuje za visinske razlike veće od 120 cm: unutarnje dimenzije najmanje 110 x 140 cm, vrata svijetlog otvora najmanje 90 cm (posmična ili prema van), pozivna ploča na 90–120 cm, rukohvat na 90 cm, taktilna crta vođenja širine 40 cm od ulaza. Ista nastupna ploha (110 x 140) vrijedi za vertikalno podiznu platformu.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)",
      freq: 1
    },
    {
      q: "Do koje visine moraju biti zatvorene bočne stranice vertikalno podizne platforme?",
      options: [
        "90 cm",
        "100 cm",
        "150 cm",
        "120 cm"
      ],
      correct: 3,
      explanation: "Vertikalno podizna platforma koristi se za visinske razlike veće od 120 cm kad se ne rješavaju dizalom; nastupna ploha najmanje 110 x 140 cm, bočne stranice zatvorene do visine 120 cm, ulazna vrata svijetlog otvora najmanje 90 cm.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)",
      freq: 1
    },
    {
      q: "Koje su najmanje dimenzije vrata pristupačnog ulaznog prostora u građevinu?",
      options: [
        "Jednokrilna 90/210 cm ili dvokrilna 2 x 80/210 cm",
        "Jednokrilna 100/200 cm ili dvokrilna 2 x 90/200 cm",
        "Jednokrilna 110/210 cm ili dvokrilna 2 x 90/210 cm",
        "Jednokrilna 120/220 cm ili dvokrilna 2 x 100/220 cm"
      ],
      correct: 2,
      explanation: "Ulazni prostor mora imati jednokrilna vrata svijetlog otvora najmanje 110/210 cm ili dvokrilna najmanje 2 x 90/210 cm, koja se otvaraju prema van ili posmično, prag do 2 cm, vjetrobran duljine 240 cm ili toplinski zastor te osvjetljenje 200 luxa.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)",
      freq: 1
    },
    {
      q: "Kako se izvodi pristupačna taktilna površina?",
      options: [
        "Reljefnom obradom visine do 5 mm, prepoznatljivom na dodir stopala ili bijelog štapa, koja ne otežava kretanje kolica i ne zadržava vodu",
        "Reljefnom obradom visine 10–15 mm od gumenih ploča",
        "Kontrastnom bojom bez reljefa",
        "Utorima dubine 20 mm okomito na smjer kretanja"
      ],
      correct: 0,
      explanation: "Taktilna površina izvodi se reljefnom obradom visine do 5 mm tako da ne otežava kretanje invalidskih kolica, da je prepoznatljiva stopalom ili bijelim štapom, ne zadržava vodu, snijeg i prljavštinu i lako se održava. Obvezna je npr. pred stubištem, do dizala, WC-a, šaltera, na peronima i pješačkim prijelazima.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)",
      freq: 1
    },
    {
      q: "Koji su obvezni elementi pristupačnosti u javnom prometu?",
      options: [
        "Rampa, stubište, dizalo i platforma",
        "Stajalište i peron, parkirališno mjesto, javna pješačka površina, semafor, pješački prijelaz, pješački otok i raskrižje",
        "Šalter, pult, bankomat i oglasni pano",
        "Kabina za presvlačenje, tuš kabina i ulaz u vodu na plaži"
      ],
      correct: 1,
      explanation: "Obvezni elementi dijele se na elemente za svladavanje visinskih razlika, elemente za neovisno življenje (ulazni prostor, WC, kuhinja, šalter, bankomat...) i elemente u javnom prometu: stajalište i peron, parkirališno mjesto, javna pješačka površina, semafor, pješački prijelaz, pješački otok i raskrižje.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)",
      freq: 1
    },
    {
      q: "Koliko pristupačnih parkirališnih mjesta mora se izvesti na javnom parkiralištu i u javnoj garaži?",
      options: [
        "10 %, ali ne manje od dva",
        "2 %, ali ne manje od jednoga",
        "Najmanje tri neovisno o veličini parkirališta",
        "5 %, ali ne manje od jednoga"
      ],
      correct: 3,
      explanation: "Na javnom parkiralištu, u javnoj garaži te uz građevinu s obveznom primjenom elemenata pristupačnosti izvodi se 5 % pristupačnih parkirališnih mjesta, ali ne manje od jednoga.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)"
    },
    {
      q: "Koliko jednostavno prilagodljivih stanova mora imati stambena zgrada na koju se primjenjuje obveza pristupačnosti?",
      options: [
        "Po 1 na svakih 10 stanova, u zgradama s 10 i više stanova",
        "Po 1 na svakih 5 stanova, u zgradama s 5 i više stanova",
        "Po 2 na svakih 10 stanova, u zgradama s 20 i više stanova",
        "Najmanje 10 % stanova u svakoj višestambenoj zgradi"
      ],
      correct: 0,
      explanation: "Obveza vrijedi za stambene i stambeno-poslovne zgrade s 10 i više stanova; na svakih 10 stanova mora se osigurati 1 jednostavno prilagodljiv stan (ulazna vrata 110 cm, ostala 80 cm, hodnici 120 cm, pragovi do 2 cm), u pravilu u prizemlju.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)"
    },
    {
      q: "Uz čiju je suglasnost dopušteno iznimno odstupiti od propisanih elemenata pristupačnosti pri rekonstrukciji građevine u urbanističkoj cjelini upisanoj u Registar kulturnih dobara?",
      options: [
        "Uz suglasnost jedinice lokalne samouprave",
        "Uz suglasnost ministarstva nadležnog za graditeljstvo, uz mišljenje tijela nadležnog za zaštitu kulturnih dobara",
        "Uz suglasnost konzervatorskog odjela bez sudjelovanja ministarstva",
        "Uz suglasnost Hrvatske komore arhitekata"
      ],
      correct: 1,
      explanation: "Ako se pri rekonstrukciji u zaštićenoj urbanističkoj cjelini ne mogu osigurati elementi pristupačnosti bez narušavanja spomeničkog svojstva, dopušteno je odstupanje uz suglasnost ministarstva nadležnog za graditeljstvo; uz prijedlog se prilaže mišljenje tijela nadležnog za zaštitu kulturnih dobara.",
      source: "Tehnički propis o osiguranju pristupačnosti (NN 12/23)"
    },
    {
      q: "Koje je tijelo u usklađenom području nadležno za donošenje europskog dokumenta za ocjenjivanje i izdavanje europske tehničke ocjene?",
      options: [
        "Prijavljeni laboratorij",
        "Tijelo za certificiranje kontrole tvorničke proizvodnje",
        "Odobreno tijelo",
        "Tijelo za tehničko ocjenjivanje (TAB)"
      ],
      correct: 3,
      explanation: "TAB (technical assessment body, imenovano tijelo) donosi europski dokument za ocjenjivanje i na temelju njega izdaje europsku tehničku ocjenu za proizvode koji nisu obuhvaćeni usklađenim normama. Prijavljena tijela (laboratorij, tijelo za certificiranje KTP, tijelo za certificiranje proizvoda) provode zadaće treće strane u usklađenom području.",
      source: "Uredba (EU) o građevnim proizvodima; Zakon o građevnim proizvodima",
      freq: 2
    },
    {
      q: "Koje tijelo provodi zadaće treće strane u NEusklađenom području građevnih proizvoda?",
      options: [
        "Prijavljeno tijelo",
        "Odobreno tijelo, koje izdaje certifikat o stalnosti svojstava ili certifikat o sukladnosti kontrole tvorničke proizvodnje prema hrvatskim normama",
        "TAB",
        "Hrvatska akreditacijska agencija"
      ],
      correct: 1,
      explanation: "U neusklađenom području odobreno tijelo provodi ocjenjivanje i provjeru stalnosti svojstava za proizvode obuhvaćene hrvatskim normama (ispitivanja, početni pregled pogona i KTP, kontinuirani nadzor, ispitivanje slučajnih uzoraka), a hrvatsko tijelo za tehničko ocjenjivanje izdaje hrvatsku tehničku ocjenu.",
      source: "Zakon o građevnim proizvodima",
      freq: 2
    },
    {
      q: "Tko provodi inspekcijski nadzor nad stavljanjem građevnih proizvoda na tržište, a tko nad njihovom uporabom na gradilištu?",
      options: [
        "Građevinski inspektori na tržištu; tržišni inspektori na gradilištu",
        "Tržišni inspektori Državnog inspektorata na tržištu; građevinski i drugi inspektori (požarni, okolišni, rada, sanitarni) nad uporabom",
        "HAA na tržištu; nadzorni inženjer na gradilištu",
        "Carinska uprava na tržištu; HZN na gradilištu"
      ],
      correct: 1,
      explanation: "Nadzor u vezi s proizvodnjom i stavljanjem na tržište provode tržišni inspektori Državnog inspektorata, a nadzor u vezi s uporabom proizvoda građevinski inspektori, inspektori zaštite od požara, okoliša, rada, sanitarni i drugi ovisno o nadležnosti. RAPEX je sustav brze razmjene obavijesti o opasnim proizvodima.",
      source: "Zakon o građevnim proizvodima"
    },
    {
      q: "Kod kojih je konstrukcija obvezno probno opterećenje?",
      options: [
        "Kod svih cestovnih mostova raspona 5 m i većeg",
        "Kod svih AB stropova raspona većeg od 8 m",
        "Kod cestovnih i pješačkih mostova raspona ≥15 m, željezničkih ≥10 m, konstrukcija raspona ≥30 m, tribina i javnih građevina za ≥1000 osoba, brana i spremnika ≥1000 m3",
        "Samo kod brana akumulacija i tunela"
      ],
      correct: 2,
      explanation: "Probno opterećenje definira projektant u projektu; obvezno je kod cestovnih i pješačkih mostova raspona 15 m i većeg, željezničkih 10 m i većeg, tribina i javnih građevina za 1000 i više osoba, konstrukcija raspona 30 m i većeg, kranskih staza raspona 15 m ili kranova 20 t i više, brana, spremnika 1000 m3 te spremnika opasnih tvari 20 m3 i većih.",
      source: "Tehnički propis za građevinske konstrukcije — posebna pravila za betonske konstrukcije"
    },
    {
      q: "Koliki je zaštitni pojas državne ceste mjeren od vanjskog ruba zemljišnog pojasa?",
      options: [
        "40 m",
        "25 m",
        "15 m",
        "10 m"
      ],
      correct: 1,
      explanation: "Zaštitni pojas sa svake strane: autoceste i brze ceste 40 m, državne ceste 25 m, županijske 15 m, lokalne 10 m. Za građenje u zaštitnom pojasu prethodno se traže posebni uvjeti HAC-a, HC-a ili županijske uprave za ceste.",
      source: "Zakon o cestama",
      freq: 1
    },
    {
      q: "U čijem je vlasništvu nerazvrstana cesta?",
      options: [
        "Republike Hrvatske",
        "Županije na čijem se području nalazi",
        "Jedinice lokalne samouprave na čijem se području nalazi, kao javno dobro u općoj uporabi",
        "Hrvatskih cesta d.o.o."
      ],
      correct: 2,
      explanation: "Nerazvrstana cesta je javno dobro u općoj uporabi u vlasništvu JLS; to su ceste koje nisu razvrstane kao javne, npr. ceste u gradovima s više od 35.000 stanovnika i sjedištima županija, ceste koje povezuju naselja, pristupne ceste do građevina, terminali i okretišta javnog prijevoza.",
      source: "Zakon o cestama",
      freq: 2
    },
    {
      q: "U kojim se mjerilima izrađuje Hrvatska osnovna karta (HOK)?",
      options: [
        "1:2000 i 1:5000",
        "1:5000 ili 1:10 000",
        "1:25 000 do 1:250 000",
        "1:300 000 i sitnijem"
      ],
      correct: 1,
      explanation: "Državna geodetska uprava izrađuje HOK u mjerilima 1:5000 ili 1:10 000, digitalnu ortofotokartu (DOF) 1:2000, 1:5000 i sitnije, detaljne topografske karte (DTK) 1:25 000 do 1:250 000 te pregledne topografske karte (PTK) 1:300 000 i sitnije.",
      source: "Zakon o državnoj izmjeri i katastru nekretnina"
    }
  ],
  flashcards: [
    { front: "Što uređuje Zakon o akreditaciji i koje je akreditacijsko tijelo u RH?", back: "Osnivanje i djelatnost tijela za nacionalnu službu za akreditaciju, područje akreditacije i akreditaciju u vezi s propisima o ocjenjivanju sukladnosti. Akreditacijsko tijelo je Hrvatska akreditacijska agencija (HAA) — javna ustanova koju osniva Vlada RH; akreditira laboratorije, certifikacijska i inspekcijska tijela.", freq: 2 },
    { front: "Što se utvrđuje u postupku akreditacije?", back: "Osposobljenost pravnih i fizičkih osoba za poslove za koje se daje akreditacija prema međunarodnim i europskim (ili istovrijednim nacionalnim) normama i dokumentima organizacija za akreditaciju. Akreditacija na zahtjev, ugovor, potvrda o akreditaciji — vremenski ograničena.", freq: 1 },
    { front: "Razlika akreditacije i certifikacije?", back: "Akreditacija = dokaz kompetentnosti za ispitivanja, mjerenja, certifikaciju ili inspekciju (vrh piramide; rezultat potvrda o akreditaciji). Certifikacija = potvrda sukladnosti sustava, proizvoda ili osoba s normom (npr. ISO 9001; rezultat certifikat). Akreditacijska tijela akreditiraju certifikacijska tijela.", freq: 1 },
    { front: "Što su norme i jesu li obvezne?", back: "Pisani dokument s definicijama, specifikacijama, pravilima za materijale, proizvode, procese; za opću i višekratnu uporabu, donesen konsenzusom i odobren od priznatog tijela (HZN na prijedlog tehničkih odbora). Same po sebi nisu obvezne, ali postaju obvezne kad se zakon ili tehnički propis na njih poziva.", freq: 2 },
    { front: "Ciljevi normizacije?", back: "Sigurnost proizvoda i procesa, zdravlje ljudi i okoliš; promicanje kakvoće; svrsishodna uporaba rada, materijala i energije; poboljšanje učinkovitosti, ograničenje raznolikosti, spojivost i zamjenjivost; otklanjanje tehničkih zapreka u međunarodnoj trgovini.", freq: 1 },
    { front: "Označavanje hrvatskih i preuzetih europskih normi; treba li nostrifikacija?", back: "Hrvatske HRN, preuzete europske HRN EN. Ne treba ih nostrificirati. Nastaju na 4 načina: pp (prijevod), po (izvornik s hrvatskim ovitkom), pr (objava obavijesti o prihvaćanju), izv (izvorna hrvatska norma)." },
    { front: "Što ako za građevni proizvod ne postoji norma?", back: "EU put: proizvođač traži europsku tehničku ocjenu — TAB donosi europski dokument za ocjenjivanje (EAD) i izdaje ETA. Nacionalni put (čl. 37. ZGP): HTTO uz prethodnu suglasnost Ministarstva izrađuje hrvatski dokument za ocjenjivanje i izdaje hrvatsku tehničku ocjenu.", freq: 1 },
    { front: "Što su tehnički propisi, tko ih donosi, jesu li obvezni?", back: "Propisi koji razrađuju temeljne zahtjeve za građevinu, tehnička svojstva građevnih proizvoda i druge tehničke zahtjeve; zahtjeve daju izravno ili upućivanjem na normu. Donosi ih ministar, objavljuju se u NN, OBVEZNI su. Primjeri: TP za građevinske konstrukcije, TP o građevnim proizvodima, TP za dimnjake, TP o racionalnoj uporabi energije.", freq: 1 },
    { front: "Predmet i primjena Tehničkog propisa za građevinske konstrukcije?", back: "Propisuje tehnička svojstva konstrukcija, zahtjeve za projektiranje, izvođenje, održavanje, uklanjanje i svojstva proizvoda za ugradnju. Primjenjuje se na konstrukcijske i nekonstrukcijske elemente koji moraju biti mehanički otporni i stabilni, na nove i rekonstruirane: betonske, čelične, spregnute, drvene, zidane, geotehničke, potresno otporne, aluminijske i ostale konstrukcije.", freq: 2 },
    { front: "Što predvidiva djelovanja na konstrukciju ne smiju prouzročiti (TPGK)?", back: "Rušenje cijele građevine ili dijela; neprihvatljive deformacije; oštećenja drugih dijelova, instalacija ili opreme zbog deformacija nosive konstrukcije; oštećenja nerazmjerna izvornom uzroku; vibracije koje ugrožavaju sigurnost ili izazivaju neugodu. U požaru — očuvanje nosivosti tijekom propisanog vremena." },
    { front: "Sadržaj programa kontrole i osiguranja kvalitete?", back: "Svojstva bitnih značajki proizvoda koji se ugrađuju; ispitivanja i dokazivanje uporabljivosti proizvoda izrađenih na gradilištu; ispitivanja tehničke/funkcionalne ispravnosti; uvjeti građenja; ispitivanja prije uporabe i kod pune zaposjednutosti; opis pokusnog rada; učestalost periodičnih pregleda; popis propisa i normi.", freq: 2 },
    { front: "Ako u programu kontrole nije drukčije navedeno, čime se smatraju potrebna ispitivanja?", back: "Kontrolnim ispitivanjima odnosno kontrolnim postupcima čiju provedbu određuje nadzorni inženjer.", freq: 1 },
    { front: "Što uređuje Zakon o mjeriteljstvu?", back: "Djelokrug državnih i ovlaštenih tijela u mjeriteljstvu, sustav točnih i spojivih mjerenja, sustav mjernih jedinica (SI), mjerne etalone, zahtjeve za mjerila u zakonskom mjeriteljstvu, stavljanje mjerila na tržište, ovjeravanje mjerila, pretpakovine i boce kao mjerne spremnike te mjeriteljsku inspekciju.", freq: 2 },
    { front: "Podjela mjeriteljstva?", back: "1) Zakonsko — radnje iz zakonskih zahtjeva: mjerenja, mjerne jedinice, mjerila i metode koje provode mjerodavna tijela. 2) Temeljno — istražuje stalnice prirodnih pojava: uspostava državnih etalona SI jedinica, sljedivost i jedinstvenost mjerenja usporedbom s međunarodnim etalonima, razvoj novih mjernih metoda.", freq: 1 },
    { front: "Tko obavlja poslove mjeriteljstva?", back: "Državni zavod za mjeriteljstvo (DZM — upravne stvari, nacrti zakona, ovlašćivanje, ovjeravanje, službena mjerenja, nadzor), ovlaštena tijela (ovjeravanje zakonitih mjerila kao javna ovlast) i pravne osobe — nacionalni umjerni laboratoriji." },
    { front: "Etalon, državni etalon, referentni etalon?", back: "Etalon = ostvarenje definicije veličine s iskazanom vrijednošću i mjernom nesigurnošću, referencija (npr. metar). Državni etalon = priznat odlukom mjerodavnog tijela kao temelj za dodjelu vrijednosti drugim etalonima u RH; sljediv do međunarodnih. Referentni etalon = za umjeravanje drugih etalona u organizaciji/lokaciji." },
    { front: "Umjeravanje vs. ovjeravanje mjerila?", back: "Umjeravanje: uspostavlja odnos između vrijednosti veličine etalona (s nesigurnostima) i pokazivanja mjerila, te se koristi za dobivanje rezultata. Ovjeravanje: ispitivanje i označavanje mjerila kojim se potvrđuje usklađenost s propisanim mjeriteljskim zahtjevima." },
    { front: "Sustav ocjenjivanja za toplinsko-izolacijske proizvode?", back: "Ovisi o proizvodu (dodatak ZA norme, HRN EN 13172): sustav 3 za sve primjene (svojstva osim požara); sustav 1 za proizvode s odredivom mjerom u proizvodnji koja daje viši razred reakcije na požar (A1, A2, B, C); sustav 4 kad nije potrebno ispitivanje reakcije na požar." },
    { front: "Granično stanje nosivosti i uporabivosti?", back: "GSN = granično opterećenje pri kojem konstrukcija gubi nosivost (sigurnost). GSU = pri kojem gubi uporabivost (pukotine, progibi) — nije bitno za sigurnost, ali za korištenje. Oba važna; GSU je češće stroži pa se prema njemu projektira." },
    { front: "Koeficijenti sigurnosti (parcijalni faktori)?", back: "GSN: 1,35 vlastito, 1,5 korisno opterećenje (povoljno korisno, npr. protiv odižućeg vjetra, ne uzima se). GSU: 1,0 za oba. Postoje i koeficijenti za smanjenje čvrstoće materijala i za kombinacije djelovanja." },
    { front: "Što su eurokodovi, čemu služe, što određuju?", back: "Set usklađenih europskih normi za statički proračun konstrukcija; cilj: jedinstven sustav proračuna u Europi i okvir za ujednačene specifikacije proizvoda. Članice ih obvezno prihvaćaju i izrađuju nacionalne dodatke (karte potresnih zona, opterećenje vjetrom i snijegom). EC0 osnove, EC1 djelovanja, EC2 beton, EC3 čelik, EC4 spregnute, EC5 drvo, EC6 zidane, EC7 geotehnika, EC8 potres, EC9 aluminij.", freq: 2 },
    { front: "Koji eurokod je u izradi i što se koristi dotad? Koji eurokod za vodovod i kanalizaciju?", back: "U izradi je eurokod za staklene konstrukcije — dotad tehnički propis za staklene konstrukcije. Vodovod i kanalizacija nisu određeni ni jednim eurokodom, već drugim normama. Uz eurokodove smiju se koristiti tehnički propisi i druge norme; strane norme uz dokazivanje.", freq: 1 },
    { front: "Što je beton? Je li u usklađenom području i koji je sustav?", back: "Građevni proizvod od cementa, agregata, vode i dodataka betonu. Nije u usklađenom području; sustav ocjenjivanja i provjere stalnosti svojstava 2+. Betonski čelik 1+, cement 1+, armatura prema tehničkoj specifikaciji. Sukladnost na gradilištu dokazuje se prema normi iz programa kontrole u glavnom projektu.", freq: 1 },
    { front: "Projekt betonske konstrukcije — što se dokazuje i što sadrži?", back: "Dokazuje da će građevina tijekom građenja i uporabnog vijeka (najmanje 50 god.) ispunjavati mehaničku otpornost i stabilnost, otpornost na požar (iznimno ne ako posebnim propisom nije određeno vrijeme očuvanja nosivosti) i dr. Sadrži: 1) tehnički opis, 2) proračun mehaničke otpornosti i stabilnosti (djelovanja, tlo, potres, snijeg, vjetar, požar), 3) program kontrole i osiguranja kvalitete.", freq: 1 },
    { front: "Što sadrži dostavnica betona?", back: "Podatke o betonu: razred tlačne čvrstoće, maksimalno zrno agregata, konzistenciju (npr. slijeganjem), udio klorida, razred izloženosti, vrstu cementa; količinu betona; podatke o betonari; oznaku sukladnosti »C«." },
    { front: "Posebna pravila za zidane konstrukcije — primjena i dijelovi projekta?", back: "Primjena na nosivo, fasadno, obložno, pregradno, vezno, ispunsko i protupožarno ziđe (i ziđe koje nije dio konstrukcije). Ne primjenjuje se na industrijske dimnjake, potporne zidove, propuste, mostove, brane, spremnike, dinamički opterećene i lukove, svodove, kupole. Projekt: tehnički opis, proračun mehaničke otpornosti i stabilnosti, program kontrole i osiguranja kvalitete.", freq: 1 },
    { front: "Vrste zidanih konstrukcija i uvjeti za omeđeno ziđe?", back: "Nearmirano, omeđeno, armirano i prednapeto ziđe. Omeđeno ziđe: vertikalni i horizontalni AB ili armirani zidani serklaži ploštine ≥0,02 m2 s najmanjom izmjerom 150 mm; nemaju učinak okvira, nego sa ziđem daju nosivost i duktilnost; ziđe preuzima vertikalna opterećenja. Predgotovljeno ziđe ne može biti nearmirano." },
    { front: "Tehnički zahtjevi za prozore i vrata — sustavi i svojstva?", back: "Proizvode se u pogonima izvan gradilišta; ugrađuju uz izjavu o sukladnosti. Profili neharmonizirano, staklo i okovi harmonizirano područje. Sustav 3 za vanjsku ovojnicu, sustav 1 za vrata na evakuacijskim putevima. Projektom se određuju: otpornost na vjetar, vodonepropusnost, propusnost zraka, prolazak topline, zvučna izolacija, otpornost na požar i dim; ako je vijek građevine duži — uvjeti zamjene.", freq: 2 },
    { front: "Na što se ne primjenjuje Tehnički propis za prozore i vrata?", back: "Vrata i stijene u potpunosti od stakla, rotirajuća vrata, vrata s mehaničkim pogonom, ostakljene krovne konstrukcije, ovješene fasade, garažna vrata, industrijska vrata i vrata za skloništa." },
    { front: "Tehnički propis za dimnjake — što propisuje, na što se ne primjenjuje, vrste dimnjaka?", back: "Propisuje tehnička svojstva dimnjaka, zahtjeve za projektiranje, izvođenje, uporabljivost, održavanje i proizvode za dimnjake. Ne primjenjuje se na samostojeći dimnjak i dimnjak isključivo u funkciji proizvodnog pogona. Vrste: montažni (na gradilištu od usklađenih proizvoda), predgotovljeni/sistemski (u pogonu), zidani (od punih opeka). Pričuvni dimnjak ≥200 cm2 za kruta goriva.", freq: 2 },
    { front: "Što je dimnjak i u kojem je projektu obuhvaćen?", back: "Dio građevnog sklopa u obliku vertikalnog kanala postojanog na požar čađe za siguran odvod dimnih plinova uređaja za loženje u atmosferu; svojstva usklađena s uređajem za loženje, priključnom cijevi i gorivom. Tehničko rješenje u arhitektonskom i/ili građevinskom te strojarskom projektu. Proizvodi: glineni/keramički, metalni, betonski, sistemski dimnjaci." },
    { front: "Vrste završnih radova?", back: "Teracerski, fasaderski, gipsarski, keramičarski, ličilački, soboslikarski, tapetarski, kamenorezački, podopolagački, parketarski, izolaterski, bravarski, limarski, stolarski, roletarski, staklorezački, radovi na antikorozijskoj zaštiti, radovi na spuštenim stropovima i lakim pregradama (18 vrsta).", freq: 2 },
    { front: "Energetsko svojstvo zgrade i kada se mora pribaviti energetski certifikat?", back: "Izračunata ili izmjerena energija za grijanje, hlađenje, ventilaciju, PTV i rasvjetu, izražena primarnom energijom u kWh/(m2·a). Certifikat se pribavlja prije uporabne dozvole, promjene vlasništva ili iznajmljivanja; izdaje ovlaštena osoba (ovlaštenje daje Ministarstvo); od 1. 7. 2013. obvezan za cjeline koje se prodaju." },
    { front: "U čemu se razlikuju energetski certifikati za postojeće i nove zgrade?", back: "Postojeće: obvezno prijedlog ekonomski opravdanih (troškovno učinkovitih) mjera poboljšanja na temelju energetskog pregleda — ušteda, povrat investicije. Nove: preporuke za korištenje zgrade vezano na gospodarenje energijom i očuvanje topline; izdaje se na temelju glavnog projekta, izjave izvođača, vizualnog pregleda i završnog izvješća nadzornog inženjera.", freq: 1 },
    { front: "Sadržaj energetskog certifikata?", back: "Opći podaci o zgradi, energetski razred, rok važenja, podaci o izdavatelju i sudionicima, oznaka certifikata, podaci o termotehničkim sustavima, energetske potrebe, korištenje obnovljivih izvora, prijedlog mjera, detaljnije informacije i objašnjenja. Uređeno Pravilnikom o energetskim pregledima i certificiranju.", freq: 1 },
    { front: "Svrha energetskog certifikata i tko ga javno izlaže?", back: "Informiranje vlasnika i korisnika o energetskom svojstvu zgrade/cjeline i usporedba zgrada (energetski sustavi, ovojnica). Javno izlaganje: zgrade javne namjene korisne površine veće od 250 m2 — prva stranica i stranica s mjerama/preporukama, zajedno na A3.", freq: 1 },
    { front: "Čega je pokazatelj energetski razred zgrade?", back: "Specifične godišnje potrebne toplinske energije za grijanje Q''H,nd i specifične godišnje primarne energije Eprim za referentne klimatske podatke i Algoritmom propisan režim. Kod stambenih Eprim obuhvaća grijanje, PTV i ventilaciju; kod nestambenih i rasvjetu. 8 razreda A+ (najbolji) do G.", freq: 1 },
    { front: "Zgrada gotovo nulte energije — rokovi?", back: "Zgrada vrlo visokih energetskih svojstava; gotovo nulta energija podmiruje se iz obnovljivih izvora na zgradi ili u blizini. Od 31. 12. 2020. sve nove zgrade; nove zgrade tijela javne vlasti nakon 31. 12. 2018." },
    { front: "Rampa — uvjeti (NN 12/23)?", back: "Za visinske razlike do 120 cm; nagib do 1:20 (5 %), iznimno 1:12 (8,3 %) do 76 cm; širina ≥120 cm vani, ≥90 cm unutra; odmorišni podest ≥150 cm na svakih 6 m; protuklizna površina; ograda s rukohvatima promjera 4 cm na 60 i 90 cm, produženi 30 cm sa zaobljenim završetkom; staklena ispuna obilježena; oznake pristupačnosti.", freq: 1 },
    { front: "Stubište — uvjeti pristupačnosti?", back: "Stuba ≤15 cm, gazište ≥33 cm; krak ≥110 cm unutra / ≥120 cm vani; kontrastni protuklizni rub ≥2 cm; rukohvati kontinuirani, produženi 30 cm; središnji rukohvat ako je krak ≥250 cm; prostor ispod kraka ograđen (70 cm) ili deniveliran 3 cm do visine 210 cm; taktilno polje upozorenja ≥40 cm pred prvom i iza posljednje stube.", freq: 1 },
    { front: "Dizalo i vertikalno podizna platforma — uvjeti?", back: "Dizalo obvezno za razliku >120 cm: kabina ≥110x140 cm, vrata ≥90 cm (posmična/van), ploča na 90–120 cm s Brailleom, rukohvat 90 cm, zvučna i svjetlosna najava, taktilna crta vođenja 40 cm od ulaza. Platforma (kad nema dizala): ploha ≥110x140 cm, bočne stranice do 120 cm, vrata ≥90 cm.", freq: 1 },
    { front: "Ulazni prostor — uvjeti pristupačnosti?", back: "Vrata jednokrilna ≥110/210 cm ili dvokrilna 2x90/210 cm, otvaranje van/posmično, pristupačna kvaka, prag ≤2 cm, otirač u razini poda; uz kružna vrata i zaokretna ≥90 cm; klizna ≥90/210 s prostorom 150x150 cm; oznaka na staklu >1,5 m2 na 90–160 cm; vjetrobran 240 cm ili toplinski zastor; osvjetljenje 200 lx.", freq: 1 },
    { front: "Taktilna površina — izvedba i gdje je obvezna?", back: "Reljefna obrada visine do 5 mm, ne otežava kolica, prepoznatljiva stopalom/bijelim štapom, ne zadržava vodu, lako se održava. Obvezna: stubište (pred prvom/iza posljednje stube), od ulaza do dizala, WC-a, bankomata, šaltera, pulta, orijentacijskog plana; stajalište/peron; semafor na cesti s više od 2 traka u smjeru; pješački prijelaz; otok u razini kolnika; raskrižje.", freq: 1 },
    { front: "Obvezni elementi pristupačnosti — tri skupine?", back: "1) Za svladavanje visinskih razlika: rampa, stubište, dizalo, vertikalno podizna i koso podizna sklopiva platforma. 2) Za neovisno življenje: ulazni prostor, komunikacije, WC, kupaonica, kuhinja, soba, radni prostor, stan, šalter, pult, bankomat, električne instalacije, kvake, induktivna petlja, oglasni pano... 3) U javnom prometu: stajalište i peron, parkirališno mjesto, pješačka površina, semafor, prijelaz, otok, raskrižje.", freq: 1 },
    { front: "Zaštitni pojas javnih cesta?", back: "Zemljište uz javnu cestu s ograničenjima; mjeri se od vanjskog ruba zemljišnog pojasa sa svake strane: autoceste 40 m, brze ceste 40 m, državne 25 m, županijske 15 m, lokalne 10 m. Za lokacijsku dozvolu u pojasu traže se posebni uvjeti HAC-a, HC-a ili ŽUC-a.", freq: 1 },
    { front: "Nerazvrstane ceste — što su i čije su?", back: "Ceste za promet vozilima koje svatko može koristiti, a nisu razvrstane kao javne: ceste u gradovima >35.000 stanovnika i sjedištima županija, ceste koje povezuju naselja i područja unutar gradova, terminali i okretišta javnog prijevoza, pristupne ceste do građevina. Javno dobro u općoj uporabi u vlasništvu JLS.", freq: 2 },
    { front: "Koja tijela treće strane postoje? Što su TAB-ovi?", back: "Usklađeno područje: prijavljeno tijelo (laboratorij; tijelo za certificiranje KTP; tijelo za certificiranje proizvoda) — certifikat o stalnosti svojstava / o sukladnosti KTP; TAB — tijelo za tehničko ocjenjivanje: donosi europski dokument za ocjenjivanje i izdaje europsku tehničku ocjenu. Neusklađeno: odobreno tijelo i hrvatsko tijelo za tehničko ocjenjivanje (hrvatska tehnička ocjena).", freq: 2 },
    { front: "Kada je obvezno probno opterećenje?", back: "Cestovni i pješački mostovi raspona ≥15 m; željeznički ≥10 m; tribine za ≥1000 osoba; konstrukcije raspona ≥30 m; kranske staze ≥15 m ili kranovi ≥20 t; javne građevine za ≥1000 osoba; brane; spremnici ≥1000 m3; spremnici opasnih/zapaljivih tvari ≥20 m3. Definira ga projektant u projektu." }
  ]
});
