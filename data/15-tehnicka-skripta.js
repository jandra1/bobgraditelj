window.APP_DATA = window.APP_DATA || [];
window.APP_DATA.push({
  id: "tehnicka",
  summary: [
    {
      heading: "Usklađeno (CE) i neusklađeno (C) područje — ZoGP (NN 76/13, 30/14, 130/17, 39/19, 118/20)",
      points: [
        "Usklađeno područje = tržište EU; primjenjuju se Uredba (EU) 305/2011 i ZoGP; proizvodi obuhvaćeni usklađenim (EU) normama; oznaka CE.",
        "Neusklađeno područje = tržište RH; regulirano ZoGP (Glava III.); proizvodi obuhvaćeni hrvatskim normama / tehničkim propisom; oznaka C.",
        "Usklađena (harmonizirana) tehnička specifikacija: usklađena europska norma (HRN EN s prilogom ZA) ili europski dokument za ocjenjivanje (EAD).",
        "Hrvatska tehnička specifikacija: tehnički propis odnosno neusklađena (hrvatska) norma bez ZA dodatka na koju upućuje tehnički propis, ili hrvatski dokument za ocjenjivanje (HDO).",
        "Analogije: prijavljeno tijelo ↔ odobreno tijelo; TAB (imenovano tijelo, donosi EAD, izdaje ETA) ↔ HTTO (izrađuje HDO, izdaje HTO); CE ↔ C.",
        "Sustavi ocjenjivanja i izjave o svojstvima identični su u oba područja; proizvođač sam bira područje ovisno o tržištu koje želi pokriti.",
        "Tijela u RH: prijavljena — IGH, Strojarski fakultet, SŠ Bedekovčina, Geo expert, Končar, Ascon, Alfa atest (popis u NANDO bazi EK); TAB i HTTO — IGH."
      ]
    },
    {
      heading: "Uredba (EU) br. 305/2011 (CPR)",
      points: [
        "Utvrđuje usklađene uvjete za stavljanje na tržište građevnih proizvoda; stavlja izvan snage Direktivu 89/106/EEZ; stupila na snagu 1. 7. 2013., obvezna za sve članice.",
        "Utvrđuje: sustave AVCP, obveze gospodarskih subjekata, usklađenu tehničku specifikaciju, zahtjeve i postupke za prijavljena tijela i TAB-ove.",
        "Prilog I — temeljni zahtjevi za građevine; Prilog II — postupak donošenja EAD-a; Prilog III — izjava o svojstvima (model); Prilog IV — područja proizvoda i zahtjevi za TAB-ove; Prilog V — sustavi AVCP (1+, 1, 2+, 3, 4).",
        "Uredba 568/2014 mijenja Prilog V (AVCP), Uredba 574/2014 mijenja Prilog III (sadržaj izjave o svojstvima).",
        "Stavljanje na tržište = prvo stavljanje na raspolaganje; stavljanje na raspolaganje = svaka isporuka za distribuciju, potrošnju ili uporabu u okviru trgovačke djelatnosti, s plaćanjem ili bez.",
        "Gospodarski subjekt = proizvođač, uvoznik (iz treće zemlje na tržište Unije), distributer (u opskrbnom lancu, nije proizvođač ni uvoznik) ili ovlašteni predstavnik (pisano ovlaštenje proizvođača)."
      ]
    },
    {
      heading: "Građevni proizvod, sklop i zahtjevi za stavljanje na tržište",
      points: [
        "Građevni proizvod: svaki proizvod ili sklop proizveden i stavljen na tržište radi trajne ugradnje u građevinu, čija svojstva utječu na svojstva građevine s obzirom na temeljne zahtjeve.",
        "Sklop: građevni proizvod stavljen na tržište kao skupina od najmanje dvije zasebne komponente koje se postavljaju zajedno.",
        "Oplata, skela, prozorska kupčica NISU građevni proizvodi — ne ugrađuju se trajno i nemaju utjecaj na temeljne zahtjeve.",
        "Prije stavljanja na tržište GP mora biti sukladan tehničkoj specifikaciji, imati izjavu o svojstvima, CE ili C oznaku i tehničku uputu.",
        "CE se postavlja vidljivo, čitljivo i neizbrisivo, prije stavljanja na tržište; bez izjave o svojstvima CE se ne smije postaviti. C oznaku smije staviti samo proizvođač ili ovlašteni zastupnik.",
        "Sigurni proizvod: u normalnim ili razumno predvidljivim uvjetima uporabe ne predstavlja nikakav ili samo najmanji rizik spojiv s uporabom; opasni proizvod = svaki koji nije siguran."
      ]
    },
    {
      heading: "Sustavi ocjenjivanja i provjere stalnosti svojstava (AVCP) — Prilog V Uredbe, Pravilnik NN 118/19",
      points: [
        "Pet sustava: 1+, 1, 2+, 3 i 4 (sustav 2 ukinut novim zakonom). Kontrolu tvorničke proizvodnje (KTP) u svim sustavima provodi proizvođač.",
        "Sustav 1+: najviše odgovornosti za prijavljeno tijelo (tijelo za certificiranje proizvoda) — ocjenjivanje svojstava, početni pregled pogona i KTP, kontinuirani nadzor KTP, ispitivanje slučajnih uzoraka iz pogona/skladišta → certifikat o stalnosti svojstava.",
        "Sustav 1: kao 1+, ali bez ispitivanja slučajnih uzoraka (npr. proizvodi od lameliranog drva) → certifikat o stalnosti svojstava.",
        "Sustav 2+: prijavljeno tijelo je tijelo za certificiranje KTP (početni pregled + kontinuirani nadzor), ocjenjivanje svojstava radi proizvođač → certifikat o sukladnosti KTP. Najsloženiji za proizvođača.",
        "Sustav 3: prijavljeni laboratorij ocjenjuje svojstva na dostavljenim uzorcima → izvještaj o ispitivanju; proizvođač samo KTP (npr. krovni pokrovi s opasnim tvarima).",
        "Sustav 4: sve provodi proizvođač (KTP + ocjenjivanje svojstava), prijavljeno tijelo nema zadataka (npr. vrata unutar zgrade).",
        "Ako je za proizvod izdana ETA, tijela (1+, 1, 3) i proizvođači (2+, 4) uzimaju ETA kao ocjenu svojstava i ne provode ocjenjivanje."
      ]
    },
    {
      heading: "Izjava o svojstvima (Prilog III Uredbe, izmjena Uredbom 574/2014)",
      points: [
        "Izjava o svojstvima sadrži:",
        "1. jedinstvenu identifikacijsku oznaku vrste proizvoda",
        "2. namjenu",
        "3. proizvođača",
        "4. ovlaštenog predstavnika",
        "5. sustav AVCP",
        "6.a usklađenu normu + prijavljeno tijelo / 6.b EAD + ETA + TAB + prijavljeno tijelo",
        "7. objavljena svojstva",
        "8. odgovarajuću/specifičnu tehničku dokumentaciju",
        "Mora sadržavati svojstvo barem jedne bitne značajke; izdaje se pod isključivom odgovornošću proizvođača.",
        "Sastavljanjem izjave proizvođač preuzima odgovornost za sukladnost proizvoda s objavljenim svojstvima.",
        "Specifična tehnička dokumentacija: dokazuje da su metode iz sustava AVCP zamijenjene drugim metodama uz istovrijednost rezultata.",
        "Iznimka (ZoGP čl. 27.): izjava nije obvezna za proizvod izrađen prema svojstvima iz glavnog projekta ako je: proizveden pojedinačno/po narudžbi u izvanserijskom postupku i ugrađen od proizvođača; izrađen na gradilištu; proizveden neindustrijski, tradicionalno ili za očuvanje baštine; pribavljen rušenjem za ponovnu uporabu.",
        "Proizvođač koji je takav proizvod i ugradio preuzima odgovornost izjavom upisanom u građevinski dnevnik; specifikacije takvih proizvoda su u programu kontrole i osiguranja kvalitete glavnog projekta."
      ]
    },
    {
      heading: "Preuzimanje proizvoda na gradilištu (Tehnički propis o građevnim proizvodima, čl. 3.)",
      points: [
        "Izvođač utvrđuje: je li proizvod namijenjen ugradnji u skladu s glavnim projektom; je li isporučen s valjanom dokumentacijom; jesu li svojstva i bitne značajke sukladne glavnom projektu; rok do kojeg se smije ugraditi. Upisuje se u građevinski dnevnik, dokumentacija se čuva na gradilištu.",
        "Valjana dokumentacija — usklađeno područje: izjava o svojstvima, oznaka CE, uputa i sigurnosne obavijesti.",
        "Valjana dokumentacija — neusklađeno područje: izjava o svojstvima, oznaka C i tehnička uputa.",
        "Proizvod izrađen na gradilištu: izvještaj o ispitivanju odnosno drugi dokument kojim se dokazuje ispunjavanje zahtjeva iz projekta. Sve na hrvatskom jeziku latiničnim pismom.",
        "Bez dokumentacije → ne ugrađivati, vratiti proizvođaču. S C oznakom → smije se ugrađivati u RH. Već ugrađen bez dokumentacije → uzorak, ispitivanje; nastavak odobrava nadzorni inženjer.",
        "Proizvod prije ugradnje odobrava nadzorni inženjer. Tehnička uporabivost: svojstva sukladna tehničkoj specifikaciji (izjava + CE/C) i nije istekao rok upotrebe iz tehničke upute."
      ]
    },
    {
      heading: "Obveze proizvođača (ZoGP čl. 30., Uredba čl. 11.) i proizvodi iz drugih država",
      points: [
        "Sastavlja izjavu o svojstvima, postavlja CE/C, izrađuje tehničku dokumentaciju; čuva tehničku dokumentaciju i izjavu 10 godina nakon stavljanja na tržište.",
        "Osigurava da serijska proizvodnja zadržava objavljena svojstva; proizvod nosi broj tipa/šarže/serije te ime i adresu proizvođača; uz proizvod prilaže izjavu i tehničke upute na hrvatskom jeziku latiničnim pismom.",
        "Nesukladan proizvod: bez odgode popravne mjere, povlačenje ili opoziv; ako predstavlja rizik — obavijestiti Ministarstvo i Državni inspektorat.",
        "Proizvod iz druge članice EU, Turske ili EFTA/EGP države na koji se ne odnosi Uredba 305/2011: uzajamno priznavanje po Uredbi (EU) 2019/515; gospodarski subjekt osigurava dokumentaciju na hrvatskom (namjeravana uporaba, značajke, sigurnosne obavijesti, upute).",
        "Ako ne dostavi izjavu o uzajamnom priznavanju — dostavlja dokumentaciju prema čl. 5. st. 5. i 6. Uredbe 2019/515."
      ]
    },
    {
      heading: "Prijavljena tijela, TAB-ovi, odobrena tijela i Ministarstvo (ZoGP čl. 12., 16.)",
      points: [
        "Ministarstvo je: kontaktna točka za građevne proizvode, upravno tijelo za prijavljivanje prijavljenih tijela, upravno tijelo za imenovanje TAB-ova, član Stalnog odbora za graditeljstvo.",
        "Rješenje o ispunjavanju zahtjeva za prijavljeno tijelo donosi Ministarstvo po mišljenju Povjerenstva za građevne proizvode; zahtjevi se dokazuju potvrdom o akreditaciji nacionalnog akreditacijskog tijela; nakon pravomoćnosti prijava EK i članicama; privremeno rješenje ako je tijelo već prijavljeno, a akreditacija u tijeku.",
        "Vrste prijavljenih tijela: laboratorij (sustav 3); laboratorij za reakciju/otpornost na požar, vanjski požar, akustiku, emisiju opasnih tvari; tijelo za certificiranje KTP (2+); tijelo za certificiranje proizvoda (1+, 1).",
        "Zahtjevi za prijavljena tijela (Uredba): pravna osobnost, neovisna treća strana (ne projektant, proizvođač, dobavljač, instalater, kupac, vlasnik, korisnik...), naknade ne ovise o broju/rezultatu ocjenjivanja, osiguranje od odgovornosti, profesionalna tajna.",
        "TAB: zahtjev Ministarstvu, provjera prema Tablici 2 Priloga IV Uredbe, rješenje po mišljenju Povjerenstva, podaci EK i članicama.",
        "Tijelo koje prestane ispunjavati zahtjeve obavještava Ministarstvo najkasnije u 15 dana; slijedi rok za uklanjanje nedostataka ili povlačenje/ograničenje/obustava prijave odnosno imenovanja.",
        "Povjerenstvo za građevne proizvode: članove imenuje ministar; savjetuje kod prijavljivanja i ukidanja rješenja, surađuje u nadzoru, savjetuje glede Stalnog odbora."
      ]
    },
    {
      heading: "Primjer izjave o svojstvima za beton (HRN EN 206): C12/15 d31.5 S2 Cl 0,40 X0",
      points: [
        "C12/15 — razred tlačne čvrstoće: fck,valj = 12 N/mm² (valjak 15/30 cm), fck,cube = 15 N/mm² (kocka 15 cm) pri 28 dana; najmanje 95 % rezultata ≥ propisane (5 % fraktil).",
        "d31.5 — maksimalno zrno agregata 31,5 mm; S2 — razred obradivosti (slijeganje, slump); Cl 0,40 — sadržaj klorida u % mase cementa; X0 — razred izloženosti (nema rizika od korozije).",
        "Razredi izloženosti: XC karbonatizacija (XC1–XC4), XF smrzavanje/odmrzavanje (XF1–XF4), XD kloridi koji nisu iz mora (XD1–XD3), XS kloridi iz mora (XS1–XS3), XA kemijsko djelovanje (XA1–XA3), XM habanje (XM1–XM3).",
        "HRN EN 206 za svaki razred okoliša propisuje max v/c, min razred tlačne čvrstoće, min količinu cementa, min količinu zraka, agregat otporan na smrzavanje (XF) i sulfatnootporni cement (XA)."
      ]
    }
  ],
  questions: [
    {
      q: "Što, uz sustave ocjenjivanja i provjere stalnosti svojstava građevnih proizvoda, uređuje Zakon o građevnim proizvodima?",
      options: [
        "Zahtjeve za prijavljena i odobrena tijela, postupak prijave i njihove obveze te primjenu Uredbe 305/2011 u usklađenom i neusklađenom području",
        "Uvjete za obavljanje djelatnosti građenja i stručnog nadzora",
        "Sadržaj i obveznu opremu glavnog projekta",
        "Postupak tehničkog pregleda i izdavanja uporabne dozvole"
      ],
      correct: 0,
      explanation: "ZoGP uređuje sustave ocjenjivanja i provjere stalnosti svojstava, radnje proizvođača i odobrenih tijela, dokumente ocjenjivanja, zahtjeve za prijavljena i odobrena tijela, postupak prijave i njihove obveze te primjenu Uredbe 305/2011 (usklađeno i neusklađeno područje).",
      source: "Zakon o građevnim proizvodima (NN 76/13, 30/14, 130/17, 39/19, 118/20)",
      freq: 2
    },
    {
      q: "Na koje se tržište odnosi neusklađeno područje građevnih proizvoda (oznaka C)?",
      options: [
        "Tržište cijele Europske unije",
        "Tržište Republike Hrvatske",
        "Tržište država EFTA-e",
        "Tržište svih članica Stalnog odbora za graditeljstvo"
      ],
      correct: 1,
      explanation: "Usklađeno područje (CE) pokriva tržište EU, dok se neusklađeno područje (C) odnosi na proizvode koji se stavljaju na tržište unutar granica RH i regulirano je Zakonom o građevnim proizvodima.",
      source: "Zakon o građevnim proizvodima, Glava III.",
      freq: 1
    },
    {
      q: "Što podrazumijeva usklađena (harmonizirana) tehnička specifikacija?",
      options: [
        "Tehnički propis i hrvatsku normu na koju on upućuje",
        "Hrvatski dokument za ocjenjivanje (HDO) i hrvatsku tehničku ocjenu",
        "Bilo koju HRN normu objavljenu u Hrvatskom zavodu za norme",
        "Usklađenu europsku normu (HRN EN s prilogom ZA) ili europski dokument za ocjenjivanje (EAD)"
      ],
      correct: 3,
      explanation: "Usklađena tehnička specifikacija je usklađena europska norma — HRN EN norma koja u prilogu ima ZA dodatak — ili europski dokument za ocjenjivanje (EAD) kada ne postoji primjenjiva norma.",
      source: "Zakon o građevnim proizvodima; Uredba (EU) 305/2011",
      freq: 2
    },
    {
      q: "Što podrazumijeva hrvatska tehnička specifikacija?",
      options: [
        "Tehnički propis odnosno neusklađenu hrvatsku normu (bez ZA dodatka) na koju upućuje tehnički propis, ili hrvatski dokument za ocjenjivanje (HDO)",
        "Europski dokument za ocjenjivanje preveden na hrvatski jezik",
        "Usklađenu europsku normu s prilogom ZA",
        "Europsku tehničku ocjenu koju je izdao IGH kao TAB"
      ],
      correct: 0,
      explanation: "Hrvatska tehnička specifikacija je tehnički propis, odnosno neusklađena (hrvatska) norma bez ZA dodatka na koju upućuje tehnički propis, ili HDO na osnovu kojeg HTTO izdaje hrvatsku tehničku ocjenu.",
      source: "Zakon o građevnim proizvodima",
      freq: 2
    },
    {
      q: "Po čemu se prepoznaje usklađena (harmonizirana) europska norma za građevne proizvode?",
      options: [
        "Po oznaci HRN ispred broja norme",
        "Po tome što u prilogu ima ZA dodatak",
        "Po tome što ju je odobrilo Povjerenstvo za građevne proizvode",
        "Po tome što je objavljena u Narodnim novinama"
      ],
      correct: 1,
      explanation: "Usklađena europska norma je HRN EN norma koja u prilogu ima ZA dodatak; norme bez ZA dodatka su neusklađene (hrvatske) norme.",
      source: "Zakon o građevnim proizvodima",
      freq: 2
    },
    {
      q: "Tko donosi europski dokument za ocjenjivanje (EAD)?",
      options: [
        "Europska komisija",
        "Prijavljeno tijelo za certificiranje proizvoda",
        "Tijelo za tehničko ocjenjivanje (TAB), odnosno organizacija TAB-ova",
        "Hrvatsko tijelo za tehničko ocjenjivanje (HTTO)"
      ],
      correct: 2,
      explanation: "U usklađenom području tijelo za tehničko ocjenjivanje (TAB, imenovano tijelo) donosi EAD i na temelju njega izdaje europsku tehničku ocjenu (ETA); u neusklađenom području HTTO izrađuje HDO.",
      source: "Uredba (EU) 305/2011; Zakon o građevnim proizvodima"
    },
    {
      q: "Tko izrađuje hrvatski dokument za ocjenjivanje (HDO) i pod kojim uvjetom?",
      options: [
        "Hrvatski zavod za norme, po nalogu Državnog inspektorata",
        "Odobreno tijelo za certificiranje proizvoda, na zahtjev investitora",
        "Ministarstvo, po prethodnom mišljenju Povjerenstva",
        "Hrvatsko tijelo za tehničko ocjenjivanje (HTTO), na traženje proizvođača i uz prethodnu suglasnost Ministarstva"
      ],
      correct: 3,
      explanation: "HDO izrađuje HTTO na traženje proizvođača uz prethodnu suglasnost Ministarstva, za proizvod koji nije (u cijelosti) obuhvaćen usklađenom tehničkom specifikacijom, tehničkim propisom ili hrvatskom normom. Ministar pravilnikom propisuje sadržaj i pravila izrade HDO-a.",
      source: "Zakon o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Kojem tijelu u usklađenom području je analogno »odobreno tijelo« iz neusklađenog područja?",
      options: [
        "Prijavljenom tijelu (notified body)",
        "Tijelu za tehničko ocjenjivanje (TAB)",
        "Nacionalnom akreditacijskom tijelu",
        "Stalnom odboru za graditeljstvo"
      ],
      correct: 0,
      explanation: "Pravila u neusklađenom području slična su usklađenom: odobreno tijelo je analogno prijavljenom tijelu, HTTO je analogan TAB-u, hrvatska tehnička specifikacija usklađenoj, a oznaka C oznaci CE.",
      source: "Zakon o građevnim proizvodima, Glava III.",
      freq: 1
    },
    {
      q: "Tko i na temelju čega izdaje hrvatsku tehničku ocjenu (HTO)?",
      options: [
        "Ministarstvo na temelju tehničkog propisa",
        "Hrvatsko tijelo za tehničko ocjenjivanje (HTTO) na temelju hrvatskog dokumenta za ocjenjivanje (HDO)",
        "Odobreni laboratorij na temelju izvještaja o ispitivanju",
        "TAB na temelju europskog dokumenta za ocjenjivanje"
      ],
      correct: 1,
      explanation: "Hrvatska tehnička ocjena je dokumentirana ocjena svojstava građevnog proizvoda u skladu s HDO-om; izdaje ju HTTO (u RH: IGH) na temelju HDO-a.",
      source: "Zakon o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Koje tijelo u Republici Hrvatskoj je imenovano kao tijelo za tehničko ocjenjivanje (TAB) i ujedno je HTTO?",
      options: [
        "Hrvatski zavod za norme",
        "Hrvatska akreditacijska agencija",
        "Institut IGH",
        "Strojarski fakultet"
      ],
      correct: 2,
      explanation: "Imenovano tijelo TAB u RH je IGH, a IGH je ujedno i hrvatsko tijelo za tehničko ocjenjivanje (HTTO). Strojarski fakultet je prijavljeno/odobreno tijelo, ne TAB.",
      source: "Skripta — Tehnička regulativa (popis tijela)"
    },
    {
      q: "Gdje se nalazi službeni popis prijavljenih tijela (notified bodies) za građevne proizvode?",
      options: [
        "U Narodnim novinama, u prilogu ZoGP-a",
        "U registru Hrvatske gospodarske komore",
        "Na stranicama Hrvatskog zavoda za norme",
        "U NANDO bazi podataka Europske komisije"
      ],
      correct: 3,
      explanation: "Lista prijavljenih tijela (IGH, Strojarski fakultet, SŠ Bedekovčina, Geo expert, Končar, Ascon institut, Alfa atest...) nalazi se u NANDO bazi podataka na stranici Europske komisije.",
      source: "Skripta — Tehnička regulativa (popis tijela)"
    },
    {
      q: "Koji dokument izdaje prijavljeno tijelo za certificiranje proizvoda u sustavima 1+ i 1?",
      options: [
        "Certifikat o stalnosti svojstava",
        "Certifikat o sukladnosti kontrole tvorničke proizvodnje",
        "Izvještaj o ispitivanju građevnog proizvoda",
        "Europsku tehničku ocjenu"
      ],
      correct: 0,
      explanation: "Na temelju usklađene tehničke specifikacije prijavljeno tijelo za certificiranje proizvoda izdaje certifikat o stalnosti svojstava (sustavi 1+ i 1); u sustavu 2+ izdaje se certifikat o sukladnosti KTP, a u sustavu 3 izvještaj o ispitivanju.",
      source: "Uredba (EU) 305/2011, Prilog V; Zakon o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Koji dokument i koje tijelo izdaje u sustavu ocjenjivanja 2+?",
      options: [
        "Certifikat o stalnosti svojstava — tijelo za certificiranje proizvoda",
        "Certifikat o sukladnosti kontrole tvorničke proizvodnje — tijelo za certificiranje kontrole tvorničke proizvodnje",
        "Izvještaj o ispitivanju — prijavljeni laboratorij",
        "Hrvatsku tehničku ocjenu — HTTO"
      ],
      correct: 1,
      explanation: "U sustavu 2+ prijavljeno (odnosno odobreno) tijelo za certificiranje kontrole tvorničke proizvodnje izdaje certifikat o sukladnosti kontrole tvorničke proizvodnje na temelju početnog pregleda pogona i kontinuiranog nadzora KTP.",
      source: "Uredba (EU) 305/2011, Prilog V; Pravilnik NN 118/19",
      freq: 1
    },
    {
      q: "U sustavu ocjenjivanja 3 prijavljeno tijelo je:",
      options: [
        "tijelo za certificiranje proizvoda koje izdaje certifikat o stalnosti svojstava",
        "tijelo za certificiranje KTP koje provodi početni pregled pogona",
        "ispitni laboratorij koji izdaje izvještaj o ispitivanju na dostavljenim uzorcima",
        "tijelo za tehničko ocjenjivanje koje izdaje ETA"
      ],
      correct: 2,
      explanation: "U sustavu 3 prijavljeno tijelo je ispitni laboratorij koji ocjenjuje svojstva na dostavljenim uzorcima i izdaje izvještaj o ispitivanju; proizvođač provodi samo kontrolu tvorničke proizvodnje (npr. krovni pokrovi koji podliježu propisima o opasnim tvarima).",
      source: "Uredba (EU) 305/2011, Prilog V",
      freq: 2
    },
    {
      q: "Što proizvođač preuzima sastavljanjem izjave o svojstvima?",
      options: [
        "Obvezu ugradnje proizvoda u građevinu",
        "Obvezu akreditacije vlastitog laboratorija",
        "Jamstvo za građevinu u koju je proizvod ugrađen",
        "Odgovornost za sukladnost građevnog proizvoda s objavljenim svojstvima"
      ],
      correct: 3,
      explanation: "Izjava o svojstvima sadrži informacije o proizvođaču, proizvodu, svojstvima i namjeravanoj uporabi; njezinim sastavljanjem proizvođač preuzima odgovornost za sukladnost građevnog proizvoda s objavljenim svojstvima.",
      source: "Uredba (EU) 305/2011; Zakon o građevnim proizvodima"
    },
    {
      q: "Koje od navedenih tijela NIJE vrsta prijavljenog tijela za koju Ministarstvo donosi rješenje prema ZoGP-u (čl. 12.)?",
      options: [
        "Laboratorij",
        "Tijelo za certificiranje kontrole tvorničke proizvodnje",
        "Tijelo za certificiranje proizvoda",
        "Tijelo za tehničko ocjenjivanje (TAB)"
      ],
      correct: 3,
      explanation: "Rješenje o ispunjavanju zahtjeva za prijavljeno tijelo može se donijeti za laboratorij, laboratorij za požarne/akustične značajke i opasne tvari, tijelo za certificiranje KTP i tijelo za certificiranje proizvoda. TAB je imenovano tijelo (čl. 16.), ne prijavljeno.",
      source: "Zakon o građevnim proizvodima, čl. 12. i 16."
    },
    {
      q: "Za koje bitne značajke laboratorij može biti prijavljen bez upute na odgovarajuću usklađenu tehničku specifikaciju?",
      options: [
        "Reakcija na požar, otpornost na požar, svojstva pri vanjskom požaru, akustična svojstva i emisija opasnih tvari",
        "Tlačna čvrstoća, vlačna čvrstoća i modul elastičnosti",
        "Toplinska provodljivost i vodonepropusnost",
        "Trajnost, otpornost na habanje i otpornost na smrzavanje"
      ],
      correct: 0,
      explanation: "Rješenje se može donijeti za laboratorij za radnje ocjenjivanja u odnosu na bitne značajke: reakcije na požar, otpornosti na požar, svojstva prilikom vanjskog požara, akustična svojstva odnosno emisija opasnih tvari — to su primjeri kad nije potrebna uputa na usklađenu tehničku specifikaciju.",
      source: "Zakon o građevnim proizvodima, čl. 12."
    },
    {
      q: "Tko izdaje rješenje na temelju kojeg odobreno tijelo provodi zadaće treće strane u neusklađenom području?",
      options: [
        "Europska komisija",
        "Ministarstvo nadležno za graditeljstvo (MGiPU)",
        "Hrvatska akreditacijska agencija",
        "Hrvatski zavod za norme"
      ],
      correct: 1,
      explanation: "Odobrena tijela su tijela koja na temelju rješenja Ministarstva graditeljstva provode zadaće treće strane u procesu ocjenjivanja i provjere stalnosti svojstava u neusklađenom području.",
      source: "Zakon o građevnim proizvodima, Glava III."
    },
    {
      q: "Što prema Uredbi 305/2011 znači »stavljanje na tržište« građevnog proizvoda?",
      options: [
        "Svaka isporuka proizvoda distributeru",
        "Ugradnja proizvoda u građevinu",
        "Prvo stavljanje proizvoda na raspolaganje na tržištu",
        "Izdavanje izjave o svojstvima"
      ],
      correct: 2,
      explanation: "Stavljanje na tržište je prvo stavljanje na raspolaganje proizvoda na tržištu, dok je stavljanje na raspolaganje svaka isporuka proizvoda za distribuiranje, potrošnju ili uporabu u okviru trgovačke djelatnosti.",
      source: "Uredba (EU) 305/2011, čl. 2.",
      freq: 1
    },
    {
      q: "Koja tvrdnja o »stavljanju na raspolaganje na tržištu« prema Uredbi 305/2011 je točna?",
      options: [
        "Odnosi se samo na isporuke uz naplatu",
        "Odnosi se samo na prvi ulazak proizvoda na tržište EU",
        "Odnosi se samo na proizvode uvezene iz trećih zemalja",
        "Svaka je isporuka za distribuiranje, potrošnju ili uporabu u okviru trgovačke djelatnosti, s plaćanjem ili bez plaćanja"
      ],
      correct: 3,
      explanation: "Stavljanje na raspolaganje na tržištu je svaka isporuka proizvoda za distribuiranje, potrošnju ili uporabu na tržištu u okviru trgovačke djelatnosti, s plaćanjem ili bez plaćanja.",
      source: "Uredba (EU) 305/2011, čl. 2.",
      freq: 1
    },
    {
      q: "Što je građevni proizvod prema Uredbi (EU) 305/2011?",
      options: [
        "Svaki proizvod ili sklop proizveden i stavljen na tržište radi trajne ugradnje u građevinu, čija svojstva utječu na svojstva građevine s obzirom na temeljne zahtjeve",
        "Svaki proizvod koji se koristi na gradilištu tijekom građenja",
        "Svaki proizvod koji ima izjavu o svojstvima",
        "Svaki proizvod za koji je izdana europska tehnička ocjena"
      ],
      correct: 0,
      explanation: "Građevni proizvod je proizvod ili sklop proizveden i stavljen na tržište radi trajne ugradnje u građevinu ili njezine dijelove (postaje sastavni dio i ne može se odvojiti bez utjecaja na temeljne zahtjeve), čija svojstva imaju učinak na svojstva građevine s obzirom na temeljne zahtjeve.",
      source: "Uredba (EU) 305/2011, čl. 2.",
      freq: 2
    },
    {
      q: "Što je »sklop« prema Uredbi (EU) 305/2011?",
      options: [
        "Građevina sastavljena od montažnih elemenata",
        "Građevni proizvod koji je proizvođač stavio na tržište kao skupinu od najmanje dvije zasebne komponente koje se postavljaju zajedno radi ugradnje",
        "Skup svih građevnih proizvoda isporučenih na gradilište u jednoj isporuci",
        "Proizvod izrađen na gradilištu od više komponenti"
      ],
      correct: 1,
      explanation: "Sklop je građevni proizvod koji je pojedini proizvođač stavio na tržište kao skupinu od najmanje dvije zasebne komponente koje je potrebno postaviti zajedno kako bi se ugradile u građevinu.",
      source: "Uredba (EU) 305/2011, čl. 2.",
      freq: 2
    },
    {
      q: "Je li oplata ili skela građevni proizvod u smislu Uredbe 305/2011?",
      options: [
        "Da, jer se koriste pri građenju građevine",
        "Da, ali samo ako imaju CE oznaku",
        "Ne, jer se ne ugrađuju trajno u građevinu i nemaju utjecaj na ispunjavanje temeljnih zahtjeva",
        "Samo skela je građevni proizvod, oplata nije"
      ],
      correct: 2,
      explanation: "Oplata, skela i prozorska kupčica nisu građevni proizvodi jer se ne ugrađuju trajno u građevinu i nemaju utjecaj na ispunjavanje temeljnih zahtjeva za građevinu.",
      source: "Uredba (EU) 305/2011, čl. 2. (definicija građevnog proizvoda)"
    },
    {
      q: "Koje zahtjeve mora ispunjavati građevni proizvod prije stavljanja na tržište?",
      options: [
        "Mora imati uporabnu dozvolu i atest Državnog inspektorata",
        "Mora biti ispitan u akreditiranom laboratoriju bez obzira na sustav ocjenjivanja",
        "Mora imati samo tehničku uputu na hrvatskom jeziku",
        "Mora biti sukladan tehničkoj specifikaciji, imati izjavu o svojstvima, CE ili C oznaku i tehničku uputu"
      ],
      correct: 3,
      explanation: "Prije stavljanja na tržište građevni proizvod mora biti sukladan zahtjevima tehničke specifikacije, posjedovati izjavu o svojstvima izdanu od proizvođača, biti označen CE ili C oznakom i imati tehničku uputu.",
      source: "Zakon o građevnim proizvodima; Uredba (EU) 305/2011",
      freq: 2
    },
    {
      q: "Zašto se građevni proizvodi u RH, iako smo u EU, ne označavaju isključivo oznakom CE?",
      options: [
        "Jer još uvijek postoje proizvodi usklađeni samo s hrvatskom tehničkom specifikacijom (neusklađeno područje) koji nose oznaku C",
        "Jer RH ima izuzeće od primjene Uredbe 305/2011 do 2030.",
        "Jer CE oznaku smiju koristiti samo proizvođači iz starih članica EU",
        "Jer oznaka C zamjenjuje CE za sve proizvode proizvedene u RH"
      ],
      correct: 0,
      explanation: "Na proizvod se postavlja CE ili C oznaka ovisno o tome je li sukladan harmoniziranoj (usklađeno područje) ili hrvatskoj tehničkoj specifikaciji (neusklađeno područje). Nije samo CE jer još ima proizvoda usklađenih samo s hrvatskom tehničkom dokumentacijom.",
      source: "Zakon o građevnim proizvodima"
    },
    {
      q: "Što od navedenog izvođač NE mora utvrditi pri preuzimanju građevnog proizvoda na gradilištu prema Tehničkom propisu o građevnim proizvodima?",
      options: [
        "Je li proizvod namijenjen ugradnji u građevinu u skladu s glavnim projektom",
        "Je li proizvod isporučen s valjanom dokumentacijom",
        "Rok do kojega se građevni proizvod smije ugraditi",
        "Je li cijena proizvoda u skladu s troškovnikom"
      ],
      correct: 3,
      explanation: "Izvođač utvrđuje: namjenu ugradnje u skladu s glavnim projektom, valjanu dokumentaciju (čl. 3.), sukladnost svojstava i bitnih značajki s glavnim projektom te rok do kojeg se proizvod smije ugraditi. Utvrđeno upisuje u građevinski dnevnik.",
      source: "Tehnički propis o građevnim proizvodima"
    },
    {
      q: "Gdje se upisuje ono što je izvođač utvrdio pri preuzimanju građevnog proizvoda i gdje se čuva dokumentacija proizvoda?",
      options: [
        "U građevinsku knjigu; dokumentacija se dostavlja Ministarstvu",
        "U građevinski dnevnik; dokumentacija se pohranjuje na gradilištu",
        "U zapisnik o primopredaji; dokumentacija se dostavlja investitoru",
        "U izvještaj nadzornog inženjera; dokumentacija se vraća proizvođaču"
      ],
      correct: 1,
      explanation: "Utvrđeno pri preuzimanju upisuje se u građevinski dnevnik, a dokumentacija se pohranjuje na gradilištu. Ako se proizvod ne ugrađuje odmah, potrebno ga je propisno skladištiti.",
      source: "Tehnički propis o građevnim proizvodima"
    },
    {
      q: "Što čini valjanu dokumentaciju građevnog proizvoda u usklađenom području na gradilištu (čl. 3. Tehničkog propisa o građevnim proizvodima)?",
      options: [
        "Izjava o svojstvima, oznaka C i tehnička uputa",
        "Izvještaj o ispitivanju i program kontrole kvalitete",
        "Izjava o svojstvima, oznaka CE, uputa i sigurnosne obavijesti",
        "Certifikat o stalnosti svojstava i europska tehnička ocjena"
      ],
      correct: 2,
      explanation: "Za proizvod u usklađenom području valjana dokumentacija je izjava o svojstvima, oznaka CE, uputa i sigurnosne obavijesti; za neusklađeno područje izjava o svojstvima, C oznaka i tehnička uputa.",
      source: "Tehnički propis o građevnim proizvodima, čl. 3. st. 2.",
      freq: 2
    },
    {
      q: "Koja je valjana dokumentacija za građevni proizvod u neusklađenom području koji dolazi na gradilište?",
      options: [
        "Izjava o svojstvima, »C« oznaka i tehnička uputa",
        "Izjava o svojstvima, oznaka CE i sigurnosne obavijesti",
        "Hrvatska tehnička ocjena i certifikat o stalnosti svojstava",
        "Izvještaj o ispitivanju odobrenog laboratorija"
      ],
      correct: 0,
      explanation: "Za građevni proizvod u neusklađenom području valjana dokumentacija je izjava o svojstvima, »C« oznaka i tehnička uputa, sve napisano hrvatskim jezikom i latiničnim pismom.",
      source: "Tehnički propis o građevnim proizvodima, čl. 3. st. 2.",
      freq: 2
    },
    {
      q: "Čime se dokazuju svojstva građevnog proizvoda proizvedenog ili izrađenog na gradilištu za ugradnju u konkretnu građevinu?",
      options: [
        "Izjavom o svojstvima i oznakom CE",
        "Certifikatom o sukladnosti kontrole tvorničke proizvodnje",
        "Hrvatskom tehničkom ocjenom",
        "Glavnim projektom i izvještajem o ispitivanju odnosno drugim dokumentom kojim se dokazuje ispunjavanje zahtjeva iz projekta"
      ],
      correct: 3,
      explanation: "Za proizvod izrađen na gradilištu u skladu s glavnim projektom valjana dokumentacija je izvještaj o ispitivanju odnosno drugi dokument kojim se dokazuje ispunjavanje zahtjeva iz projekta; svojstva se dokazuju glavnim projektom i izvještajem o ispitivanju.",
      source: "Tehnički propis o građevnim proizvodima, čl. 3.",
      freq: 1
    },
    {
      q: "Što treba učiniti ako građevni proizvod dođe na gradilište bez propisane dokumentacije?",
      options: [
        "Ugraditi ga uz suglasnost investitora",
        "Ne smije se ugraditi, a proizvod treba vratiti proizvođaču",
        "Ugraditi ga i naknadno zatražiti dokumentaciju od distributera",
        "Ugraditi ga ako ima oznaku CE na ambalaži"
      ],
      correct: 1,
      explanation: "Proizvod bez valjane dokumentacije ne smije se ugraditi i treba ga vratiti proizvođaču.",
      source: "Tehnički propis o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Na gradilište je stigao proizvod s valjanom dokumentacijom, ali nosi oznaku C umjesto CE. Što vrijedi?",
      options: [
        "Ne smije se ugraditi jer je u RH obvezna samo CE oznaka",
        "Smije se ugraditi samo uz suglasnost Ministarstva",
        "Može se ugrađivati na gradilištu u Hrvatskoj jer je proizvod iz neusklađenog područja",
        "Mora se prethodno ispitati u prijavljenom laboratoriju"
      ],
      correct: 2,
      explanation: "Proizvod s oznakom C (neusklađeno područje) može se ugrađivati u Hrvatskoj. Ako je pak sam proizvod drugačiji od dokumentacije (npr. druge dimenzije cigle), ne smije se ugrađivati.",
      source: "Zakon o građevnim proizvodima; Tehnički propis o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Proizvod je stigao bez dokumentacije, a već se počeo ugrađivati. Što treba učiniti i tko odobrava nastavak radova?",
      options: [
        "Nastaviti ugradnju i evidentirati u građevinsku knjigu; odobrava investitor",
        "Uzeti uzorak i ispitati svojstva; ako ne odgovaraju, obustaviti gradnju i ukloniti greške; nastavak odobrava nadzorni inženjer",
        "Odmah obustaviti gradnju i prijaviti Državnom inspektoratu; nastavak odobrava inspektor",
        "Zatražiti od proizvođača naknadnu izjavu o svojstvima; nastavak odobrava projektant"
      ],
      correct: 1,
      explanation: "Uzima se uzorak i ispituju svojstva; ako ne odgovaraju, obustavlja se gradnja i uklanjaju nastale greške. Ugradnju odnosno nastavak radova odobrava nadzorni inženjer, a sve se evidentira u građevinski dnevnik uz obavijest investitoru, proizvođaču i nadzornom inženjeru.",
      source: "Tehnički propis o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Tko treba odobriti građevni proizvod koji je ocijenjen i ima izjavu o svojstvima prije nego što ga izvođač ugradi?",
      options: [
        "Nadzorni inženjer",
        "Projektant",
        "Građevinski inspektor",
        "Revident"
      ],
      correct: 0,
      explanation: "Prije ugradnje proizvod koji ima izjavu o svojstvima odobrava nadzorni inženjer.",
      source: "Tehnički propis o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Kada je građevni proizvod tehnički uporabljiv?",
      options: [
        "Kada je ugrađen u skladu s uputama proizvođača",
        "Kada ga je odobrilo Povjerenstvo za građevne proizvode",
        "Kada je proizveden u državi članici EU",
        "Kada su mu tehnička svojstva sukladna tehničkoj specifikaciji (dokazano izjavom o svojstvima i CE/C oznakom) i nije mu istekao rok upotrebe iz tehničke upute"
      ],
      correct: 3,
      explanation: "Građevni proizvod je uporabljiv ako su mu tehnička svojstva sukladna tehničkoj specifikaciji, što se dokazuje izjavom o svojstvima i CE ili C oznakom, odnosno ako mu nije istekao rok upotrebe definiran u tehničkoj uputi.",
      source: "Zakon o građevnim proizvodima; Tehnički propis o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Koji je minimalni broj svojstava građevnog proizvoda koja se mogu ispitivati?",
      options: [
        "Sva svojstva navedena u tehničkoj specifikaciji",
        "Barem tri bitne značajke",
        "Barem jedno (npr. tlačna čvrstoća ili reakcija na požar)",
        "Nijedno, ako proizvod ima CE oznaku"
      ],
      correct: 2,
      explanation: "Ispituje se barem jedno svojstvo, npr. tlačna čvrstoća ili reakcija na požar; i izjava o svojstvima mora sadržavati svojstvo barem jedne bitne značajke.",
      source: "Skripta — Tehnička regulativa; Uredba (EU) 305/2011, čl. 6."
    },
    {
      q: "Koji je sustav ocjenjivanja sukladnosti ukinut novim Zakonom o građevnim proizvodima?",
      options: [
        "Sustav 1",
        "Sustav 2",
        "Sustav 3",
        "Sustav 4"
      ],
      correct: 1,
      explanation: "Novim ZoGP-om osigurana je provedba Uredbe 305/2011, uvedena podjela na usklađeno i neusklađeno područje, novi pojmovi (izjava o svojstvima, certifikat o stalnosti svojstava, certifikat o sukladnosti KTP, bitne značajke) te je ukinut sustav ocjenjivanja 2. Ostalo je pet sustava: 1+, 1, 2+, 3 i 4.",
      source: "Zakon o građevnim proizvodima"
    },
    {
      q: "Građevni proizvod na koji se ne odnosi Uredba 305/2011 zakonito je stavljen na tržište u drugoj državi članici EU ili Turskoj. Kako se tretira u RH?",
      options: [
        "Mora proći postupak ocjenjivanja kod odobrenog tijela i dobiti oznaku C",
        "Ne smije se stavljati na tržište RH bez hrvatske tehničke ocjene",
        "Smatra se kompatibilnim u smislu Uredbe (EU) 2019/515 o uzajamnom priznavanju robe",
        "Mora imati CE oznaku izdanu od hrvatskog prijavljenog tijela"
      ],
      correct: 2,
      explanation: "Takvi proizvodi (iz članice EU, Turske ili EFTA države stranke Sporazuma o EGP-u) smatraju se kompatibilnima prema Uredbi (EU) 2019/515 o uzajamnom priznavanju robe koja se zakonito stavlja na tržište u drugoj državi članici.",
      source: "Zakon o građevnim proizvodima (NN 76/13 … 118/20)",
      freq: 1
    },
    {
      q: "Što mora osigurati gospodarski subjekt koji u RH stavlja na raspolaganje proizvod iz druge članice EU po načelu uzajamnog priznavanja (Uredba 2019/515)?",
      options: [
        "Da je proizvod ispitan u hrvatskom odobrenom laboratoriju",
        "Da prateća dokumentacija na hrvatskom jeziku latiničnim pismom sadrži namjeravanu uporabu, značajke proizvoda, sigurnosne obavijesti i upute o rukovanju, skladištenju, ugradnji i odlaganju",
        "Da proizvod ima hrvatsku tehničku ocjenu",
        "Da proizvod ima oznaku C koju postavlja uvoznik"
      ],
      correct: 1,
      explanation: "Gospodarski subjekt mora osigurati da prateća dokumentacija (na hrvatskom jeziku latiničnim pismom) navodi namjeravanu uporabu i značajke proizvoda te sadrži sigurnosne obavijesti i upute o rukovanju, skladištenju, ugradnji i trajnom odlaganju. Ako ne dostavi izjavu o uzajamnom priznavanju, dostavlja dokumentaciju prema čl. 5. st. 5. i 6. Uredbe 2019/515.",
      source: "Zakon o građevnim proizvodima; Uredba (EU) 2019/515",
      freq: 1
    },
    {
      q: "Proizvođač želi na tržište staviti novi proizvod koji nije obuhvaćen normama. Tko odlučuje hoće li ići u usklađeno ili neusklađeno područje?",
      options: [
        "Proizvođač sam bira područje, ovisno o tržištu koje želi pokriti (europsko ili samo hrvatsko)",
        "Ministarstvo rješenjem određuje područje",
        "Povjerenstvo za građevne proizvode nakon savjetovanja s HZN-om",
        "Prijavljeno tijelo koje će provoditi ocjenjivanje"
      ],
      correct: 0,
      explanation: "Proizvođač sam bira područje — to je njegov izbor ovisno o tome želi li pokriti europsko tržište (EAD/ETA, CE) ili samo hrvatsko (HDO/HTO, C).",
      source: "Skripta — Tehnička regulativa"
    },
    {
      q: "Što je tehnička specifikacija građevnog proizvoda?",
      options: [
        "Dokument kojim proizvođač izražava svojstva proizvoda u vezi s bitnim značajkama",
        "Dokument u skladu s kojim se ocjenjuju svojstva građevnog proizvoda u odnosu na njegove bitne značajke; može biti usklađena/neusklađena norma ili europski/hrvatski dokument za ocjenjivanje",
        "Ugovorni dokument između investitora i izvođača koji određuje kvalitetu radova",
        "Certifikat prijavljenog tijela o stalnosti svojstava"
      ],
      correct: 1,
      explanation: "Tehnička specifikacija je dokument prema kojem se ocjenjuju svojstva GP u odnosu na bitne značajke; sadrži minimalne zahtjeve (npr. min. tlačna čvrstoća, otpornost na požar, vodopropusnost). Može biti norma, dio norme ili poseban dokument neovisan o normi.",
      source: "Zakon o građevnim proizvodima; Uredba (EU) 305/2011",
      freq: 2
    },
    {
      q: "U kojem slučaju HTTO NE može izraditi hrvatski dokument za ocjenjivanje (HDO) za proizvod?",
      options: [
        "Proizvod ne potpada ni u jedno područje primjene postojeće usklađene tehničke specifikacije, tehničkog propisa ili hrvatske norme",
        "Za najmanje jednu bitnu značajku metoda ocjenjivanja iz postojeće specifikacije nije prikladna",
        "Postojeća specifikacija ne određuje nikakvu metodu ocjenjivanja za bar jednu bitnu značajku",
        "Proizvod je u cijelosti obuhvaćen usklađenom europskom normom, ali proizvođač želi jeftiniji postupak"
      ],
      correct: 3,
      explanation: "HDO se izrađuje samo za proizvod koji nije (u cijelosti) obuhvaćen usklađenom tehničkom specifikacijom, tehničkim propisom ili hrvatskom normom: ako ne potpada ni u jedno područje primjene, ako metoda ocjenjivanja nije prikladna za barem jednu bitnu značajku ili ako nije određena nikakva metoda za barem jednu bitnu značajku.",
      source: "Zakon o građevnim proizvodima (HDO)"
    },
    {
      q: "Kada je Uredba (EU) br. 305/2011 stupila na snagu i za koga je obvezna?",
      options: [
        "1. 1. 2011., obvezna za članice koje su je ratificirale",
        "1. 7. 2013., obvezna za sve države članice EU",
        "1. 7. 2013., obvezna samo za nove članice EU",
        "1. 1. 2015., obvezna za članice i države EFTA-e"
      ],
      correct: 1,
      explanation: "Uredba 305/2011 utvrđuje usklađene uvjete za stavljanje na tržište građevnih proizvoda, stavlja izvan snage Direktivu 89/106/EEZ, stupila je na snagu 1. 7. 2013. i obvezna je za sve države članice EU.",
      source: "Uredba (EU) br. 305/2011",
      freq: 2
    },
    {
      q: "Što sadrži Prilog V Uredbe (EU) 305/2011?",
      options: [
        "Temeljne zahtjeve za građevine",
        "Model izjave o svojstvima",
        "Sustave ocjenjivanja i provjere stalnosti svojstava (1+, 1, 2+, 3, 4)",
        "Područja proizvoda i zahtjeve za tijela za tehničko ocjenjivanje"
      ],
      correct: 2,
      explanation: "Prilog I — temeljni zahtjevi za građevine; Prilog II — postupak donošenja EAD-a; Prilog III — izjava o svojstvima; Prilog IV — područja proizvoda i zahtjevi za TAB-ove; Prilog V — ocjenjivanje i provjera stalnosti svojstava (1+, 1, 2+, 3, 4).",
      source: "Uredba (EU) br. 305/2011, Prilog V",
      freq: 2
    },
    {
      q: "U kojem se prilogu Uredbe 305/2011 nalazi model (obrazac) izjave o svojstvima?",
      options: [
        "Prilog I",
        "Prilog II",
        "Prilog IV",
        "Prilog III"
      ],
      correct: 3,
      explanation: "Izjavu o svojstvima potrebno je sastaviti koristeći model utvrđen u Prilogu III Uredbe 305/2011 (izmijenjen Uredbom 574/2014). Prilog I sadrži temeljne zahtjeve za građevine, Prilog IV područja proizvoda i zahtjeve za TAB-ove.",
      source: "Uredba (EU) br. 305/2011, Prilog III; Uredba 574/2014",
      freq: 2
    },
    {
      q: "Što mijenjaju delegirane uredbe 568/2014 i 574/2014 u odnosu na Uredbu 305/2011?",
      options: [
        "568/2014 mijenja Prilog V (sustavi AVCP), a 574/2014 Prilog III (sadržaj izjave o svojstvima)",
        "568/2014 mijenja Prilog I (temeljni zahtjevi), a 574/2014 Prilog IV (TAB-ovi)",
        "Obje mijenjaju Prilog II (postupak donošenja EAD-a)",
        "568/2014 uvodi oznaku C, a 574/2014 ukida sustav 2"
      ],
      correct: 0,
      explanation: "Uredba 568/2014 mijenja Prilog V koji određuje ocjenjivanje i provjeru stalnosti svojstava, a Uredba 574/2014 mijenja Prilog III koji određuje sadržaj izjave o svojstvima.",
      source: "Uredbe (EU) 568/2014 i 574/2014"
    },
    {
      q: "Koja tvrdnja o postavljanju oznake CE na građevni proizvod je točna?",
      options: [
        "Postavlja se nakon što je proizvod ugrađen u građevinu",
        "Smije se postaviti i bez izjave o svojstvima ako je proizvod obuhvaćen usklađenom normom",
        "Postavlja se vidljivo, čitljivo i neizbrisivo na proizvod ili etiketu, prije stavljanja na tržište, i samo ako je sastavljena izjava o svojstvima",
        "Postavlja ju prijavljeno tijelo nakon izdavanja certifikata"
      ],
      correct: 2,
      explanation: "CE (Conformité Européenne) postavlja se na proizvode za koje je proizvođač sastavio izjavu o svojstvima; bez izjave se ne smije postaviti. Postavlja se vidljivo, čitljivo i neizbrisivo na proizvod ili etiketu, prije stavljanja na tržište.",
      source: "Uredba (EU) 305/2011, čl. 8. i 9.",
      freq: 1
    },
    {
      q: "Tko smije staviti »C« oznaku na građevni proizvod?",
      options: [
        "Odobreno tijelo za certificiranje proizvoda",
        "Samo proizvođač ili njegov ovlašteni zastupnik",
        "Distributer koji proizvod stavlja na tržište RH",
        "Nadzorni inženjer prije ugradnje"
      ],
      correct: 1,
      explanation: "C oznaka je jedina oznaka koja potvrđuje sukladnost proizvoda s hrvatskom tehničkom specifikacijom; smije ju staviti samo proizvođač ili njegov ovlašteni zastupnik, i to na proizvode za koje je sastavljena izjava o svojstvima prema ZoGP-u.",
      source: "Zakon o građevnim proizvodima",
      freq: 1
    },
    {
      q: "Koji od navedenih NIJE jedan od pet sustava ocjenjivanja i provjere stalnosti svojstava građevnih proizvoda?",
      options: [
        "Sustav 1+",
        "Sustav 2+",
        "Sustav 2",
        "Sustav 4"
      ],
      correct: 2,
      explanation: "Ocjenjivanje i provjera stalnosti svojstava provodi se u skladu s pet sustava: 1+, 1, 2+, 3 i 4. Sustav 2 je ukinut.",
      source: "Uredba (EU) 305/2011, Prilog V; Pravilnik NN 118/19, prilog IX"
    },
    {
      q: "Po čemu se sustav 1+ razlikuje od sustava 1?",
      options: [
        "U sustavu 1 prijavljeno tijelo ne provodi početni pregled proizvodnog pogona",
        "U sustavu 1 ocjenjivanje svojstava provodi proizvođač, a ne prijavljeno tijelo",
        "U sustavu 1+ prijavljeno tijelo je laboratorij, a u sustavu 1 tijelo za certificiranje proizvoda",
        "U sustavu 1 prijavljeno tijelo ne provodi ispitivanje slučajnih uzoraka uzetih u proizvodnom pogonu ili skladištu proizvođača"
      ],
      correct: 3,
      explanation: "Sustav 1+ definira najviše odgovornosti za prijavljeno tijelo (tijelo za certificiranje proizvoda). Za razliku od 1+, u sustavu 1 prijavljeno tijelo ne provodi ispitivanja slučajnih uzoraka uzetih prije stavljanja na tržište (primjer: proizvodi od lameliranog drva).",
      source: "Uredba (EU) 305/2011, Prilog V",
      freq: 1
    },
    {
      q: "Koje radnje provodi proizvođač u sustavu 1+?",
      options: [
        "Kontrolu tvorničke proizvodnje i ispitivanje uzoraka koje sam uzima u pogonu prema propisanom planu ispitivanja",
        "Ocjenjivanje svojstava proizvoda i početni pregled pogona",
        "Kontinuirani nadzor kontrole tvorničke proizvodnje",
        "Ispitivanje slučajnih uzoraka iz skladišnih prostora"
      ],
      correct: 0,
      explanation: "U sustavima 1+ i 1 proizvođač provodi kontrolu tvorničke proizvodnje i ispitivanja uzoraka koje uzima u proizvodnom pogonu prema propisanom planu ispitivanja; ocjenjivanje svojstava, početni pregled, kontinuirani nadzor (i u 1+ slučajne uzorke) provodi tijelo za certificiranje proizvoda.",
      source: "Uredba (EU) 305/2011, Prilog V",
      freq: 1
    },
    {
      q: "Što je karakteristično za sustav 2+?",
      options: [
        "Prijavljeno tijelo ocjenjuje svojstva proizvoda i izdaje certifikat o stalnosti svojstava",
        "Prijavljeno tijelo (tijelo za certificiranje KTP) ne ocjenjuje svojstva proizvoda — to radi proizvođač — nego provodi početni pregled pogona i kontinuirani nadzor KTP",
        "Prijavljeno tijelo nema nikakvih zadataka",
        "Prijavljeni laboratorij ispituje dostavljene uzorke"
      ],
      correct: 1,
      explanation: "U sustavu 2+ prijavljeno tijelo ne provodi ocjenjivanje svojstava (to je zadatak proizvođača), nego kao tijelo za certificiranje KTP provodi početni pregled pogona i kontinuirani nadzor te izdaje certifikat o sukladnosti KTP. Najsloženiji je za proizvođača.",
      source: "Uredba (EU) 305/2011, Prilog V"
    },
    {
      q: "Koji je sustav ocjenjivanja u kojem prijavljeno tijelo nema nikakvih zadataka, a sve provodi proizvođač (npr. vrata unutar zgrade)?",
      options: [
        "Sustav 2+",
        "Sustav 3",
        "Sustav 1",
        "Sustav 4"
      ],
      correct: 3,
      explanation: "Sustav 4 ocjenjivanje i provjeru stalnosti svojstava u potpunosti prepušta proizvođaču (KTP i ocjenjivanje svojstava); prijavljeno tijelo nema zadataka.",
      source: "Uredba (EU) 305/2011, Prilog V"
    },
    {
      q: "Koju radnju ocjenjivanja i provjere stalnosti svojstava proizvođač provodi u SVIH pet sustava?",
      options: [
        "Ispitivanje slučajnih uzoraka iz skladišta",
        "Kontrolu tvorničke proizvodnje",
        "Početni pregled proizvodnog pogona",
        "Ocjenjivanje svojstava na temelju tabličnih vrijednosti"
      ],
      correct: 1,
      explanation: "Kontrolu tvorničke proizvodnje proizvođač provodi u svim sustavima (1+, 1, 2+, 3 i 4). Ocjenjivanje svojstava provodi proizvođač u 2+ i 4, a tijelo u 1+, 1 i 3.",
      source: "Uredba (EU) 305/2011, Prilog V; Pravilnik NN 118/19"
    },
    {
      q: "Kako postupaju prijavljena tijela i proizvođači u pogledu ocjenjivanja svojstava proizvoda za koji je izdana europska tehnička ocjena (ETA)?",
      options: [
        "Provode ponovno ocjenjivanje svojstava u okviru svog sustava",
        "Traže od TAB-a da ponovi ocjenu za svaku seriju",
        "Uzimaju ETA kao ocjenu svojstava tog proizvoda i ne izvode zadatke ocjenjivanja svojstava",
        "Zamjenjuju ETA hrvatskom tehničkom ocjenom"
      ],
      correct: 2,
      explanation: "Za proizvode s izdanom ETA prijavljena tijela (sustavi 1+, 1 i 3) i proizvođači (sustavi 2+ i 4) uzimaju europsku tehničku ocjenu kao ocjenu svojstava proizvoda te ne izvode zadatke ocjenjivanja svojstava.",
      source: "Uredba (EU) 305/2011, Prilog V"
    },
    {
      q: "Tko je »uvoznik« prema Uredbi 305/2011?",
      options: [
        "Svaka osoba u opskrbnom lancu koja stavlja proizvod na raspolaganje na tržištu",
        "Fizička ili pravna osoba s poslovnim nastanom u Uniji koja građevni proizvod iz treće zemlje stavlja na tržište Unije",
        "Osoba koja je dobila pisano ovlaštenje proizvođača da djeluje u njegovo ime",
        "Osoba koja trguje proizvodom pod svojim imenom ili žigom"
      ],
      correct: 1,
      explanation: "Uvoznik je osoba s poslovnim nastanom unutar Unije koja proizvod iz treće zemlje stavlja na tržište Unije; distributer je osoba u opskrbnom lancu (nije proizvođač ni uvoznik) koja stavlja proizvod na raspolaganje; ovlašteni predstavnik ima pisano ovlaštenje proizvođača.",
      source: "Uredba (EU) 305/2011, čl. 2."
    },
    {
      q: "Što je zajednički naziv za proizvođača, uvoznika, distributera i ovlaštenog predstavnika prema Uredbi 305/2011?",
      options: [
        "Gospodarski subjekt",
        "Sudionik u gradnji",
        "Prijavljeno tijelo",
        "Treća strana"
      ],
      correct: 0,
      explanation: "Gospodarski subjekt znači proizvođač, uvoznik, distributer ili ovlašteni predstavnik.",
      source: "Uredba (EU) 305/2011, čl. 2."
    },
    {
      q: "Koliko dugo proizvođač mora čuvati tehničku dokumentaciju i izjavu o svojstvima nakon stavljanja proizvoda na tržište?",
      options: [
        "5 godina",
        "2 godine",
        "10 godina",
        "Trajno, dok je proizvod na tržištu"
      ],
      correct: 2,
      explanation: "Proizvođač čuva tehničku dokumentaciju i izjavu o svojstvima tijekom 10 godina nakon što je građevni proizvod stavljen na tržište.",
      source: "Zakon o građevnim proizvodima, čl. 30.; Uredba (EU) 305/2011, čl. 11."
    },
    {
      q: "Koga proizvođač mora bez odgode obavijestiti ako građevni proizvod koji je stavio na tržište predstavlja rizik?",
      options: [
        "Nadzornog inženjera i investitora",
        "Ministarstvo i Državni inspektorat",
        "Europsku komisiju i NANDO bazu",
        "Povjerenstvo za građevne proizvode i HZN"
      ],
      correct: 1,
      explanation: "Ako proizvod predstavlja rizik, proizvođač je obvezan bez odgode obavijestiti Ministarstvo i Državni inspektorat, navodeći podatke o nesukladnosti i poduzetim mjerama; inače mora poduzeti popravne mjere, povući ili opozvati proizvod.",
      source: "Zakon o građevnim proizvodima, čl. 30.; Uredba (EU) 305/2011, čl. 11."
    },
    {
      q: "Koju od navedenih uloga u provedbi Uredbe 305/2011 NEMA Ministarstvo nadležno za graditeljstvo?",
      options: [
        "Kontaktna točka za građevne proizvode",
        "Upravno tijelo za prijavljivanje prijavljenih tijela i imenovanje TAB-ova",
        "Član Stalnog odbora za graditeljstvo",
        "Nacionalno akreditacijsko tijelo koje izdaje potvrde o akreditaciji"
      ],
      correct: 3,
      explanation: "Ministarstvo je kontaktna točka za građevne proizvode, upravno tijelo za prijavljivanje prijavljenih tijela, upravno tijelo za imenovanje TAB-ova i član Stalnog odbora za graditeljstvo. Akreditaciju izdaje nacionalno akreditacijsko tijelo, a njome prijavljena tijela dokazuju ispunjavanje zahtjeva.",
      source: "Zakon o građevnim proizvodima, čl. 12."
    },
    {
      q: "Čime prijavljena tijela dokazuju ispunjavanje zahtjeva Uredbe 305/2011 i tko donosi rješenje?",
      options: [
        "Potvrdom o akreditaciji nacionalnog akreditacijskog tijela; rješenje donosi Ministarstvo po prethodnom mišljenju Povjerenstva za građevne proizvode",
        "Certifikatom ISO 9001; rješenje donosi Europska komisija",
        "Mišljenjem HZN-a; rješenje donosi Državni inspektorat",
        "Ugovorom s TAB-om; rješenje donosi Stalni odbor za graditeljstvo"
      ],
      correct: 0,
      explanation: "Pravna osoba podnosi zahtjev Ministarstvu; ispunjavanje zahtjeva dokazuje potvrdom o akreditaciji nacionalnog akreditacijskog tijela. Rješenje donosi Ministarstvo po prethodno pribavljenom mišljenju Povjerenstva, a nakon pravomoćnosti podnosi prijavu Europskoj komisiji i članicama.",
      source: "Zakon o građevnim proizvodima, čl. 12."
    },
    {
      q: "Kada Ministarstvo donosi privremeno rješenje za prijavljeno tijelo?",
      options: [
        "Kada tijelo nema pravnu osobnost",
        "Kada je podnositelj već prijavljen Europskoj komisiji, a akreditacijski postupak za traženi proizvod je u tijeku",
        "Kada Povjerenstvo nije dalo mišljenje u roku",
        "Kada tijelo ima osiguranje od odgovornosti samo za dio djelatnosti"
      ],
      correct: 1,
      explanation: "Ako se utvrdi da je riječ o podnositelju koji je već prijavljen Europskoj komisiji, a akreditacijski postupak prema građevnom proizvodu za koji se podnosi zahtjev je u tijeku, donosi se privremeno rješenje.",
      source: "Zakon o građevnim proizvodima, čl. 12."
    },
    {
      q: "Koja tvrdnja o zahtjevima za prijavljena tijela prema Uredbi 305/2011 je točna?",
      options: [
        "Naknade uprave i osoblja za ocjenjivanje mogu ovisiti o broju provedenih ocjenjivanja",
        "Prijavljeno tijelo može biti proizvođač proizvoda koje ocjenjuje ako je akreditirano",
        "Prijavljeno tijelo mora imati osiguranje od odgovornosti, osim ako je odgovornost preuzela država članica",
        "Osoblje prijavljenog tijela nije vezano profesionalnom tajnom"
      ],
      correct: 2,
      explanation: "Prijavljeno tijelo ima pravnu osobnost, neovisna je treća strana (ne smije biti projektant, proizvođač, dobavljač, instalater, kupac, vlasnik, korisnik ni održavatelj), naknade ne ovise o broju ni rezultatu ocjenjivanja, ima osiguranje od odgovornosti (osim ako je odgovornost preuzela država) i osoblje čuva profesionalnu tajnu.",
      source: "Uredba (EU) 305/2011, čl. 43."
    },
    {
      q: "Prema kojem dijelu Uredbe 305/2011 Ministarstvo provjerava zahtjeve za imenovanje tijela za tehničko ocjenjivanje (TAB)?",
      options: [
        "Tablica 2 Priloga IV",
        "Prilog V",
        "Prilog III",
        "Prilog I"
      ],
      correct: 0,
      explanation: "Za imenovanje TAB-om pravna osoba podnosi zahtjev Ministarstvu koje provjerava ispunjavanje zahtjeva iz Tablice 2 Priloga IV Uredbe; rješenje se donosi po mišljenju Povjerenstva za jedno ili više područja proizvoda, a podaci se dostavljaju EK i članicama.",
      source: "Zakon o građevnim proizvodima, čl. 16.; Uredba (EU) 305/2011, Prilog IV"
    },
    {
      q: "U kojem roku prijavljeno ili imenovano tijelo mora obavijestiti Ministarstvo da je prestalo ispunjavati zahtjeve na temelju kojih je doneseno rješenje?",
      options: [
        "8 dana",
        "30 dana",
        "60 dana",
        "Najkasnije 15 dana"
      ],
      correct: 3,
      explanation: "Tijelo koje prestane ispunjavati zahtjeve dužno je obavijestiti Ministarstvo najkasnije u roku od 15 dana; nakon toga se pokreće provjera, pribavlja mišljenje Povjerenstva o ozbiljnosti nedostataka i daje rok za uklanjanje ili se provodi povlačenje, ograničenje ili obustava prijave odnosno imenovanja.",
      source: "Zakon o građevnim proizvodima (postupak ukidanja ovlaštenja)"
    },
    {
      q: "Tko imenuje članove Povjerenstva za građevne proizvode i koja mu je uloga?",
      options: [
        "Europska komisija; provodi akreditaciju prijavljenih tijela",
        "Ministar; savjetuje Ministarstvo kod prijavljivanja i ukidanja rješenja prijavljenih/odobrenih tijela, surađuje u nadzoru nad provedbom ZoGP-a i savjetuje glede Stalnog odbora za graditeljstvo",
        "Hrvatska komora inženjera građevinarstva; izdaje hrvatske tehničke ocjene",
        "Vlada RH; donosi tehničke propise"
      ],
      correct: 1,
      explanation: "Povjerenstvo obavlja poslove uređene ZoGP-om vezane uz stavljanje na tržište građevnih proizvoda: savjetuje Ministarstvo kod prijavljivanja tijela, kod sumnji u nedostatke i ukidanja rješenja, surađuje u nadzoru i savjetuje glede članstva u Stalnom odboru za graditeljstvo. Članove imenuje ministar.",
      source: "Zakon o građevnim proizvodima (Povjerenstvo za građevne proizvode)"
    },
    {
      q: "Što je certifikat o stalnosti svojstava i za koje se sustave izdaje?",
      options: [
        "Dokument kojim proizvođač izražava svojstva proizvoda; izdaje se za sustave 2+ i 4",
        "Dokument laboratorija o rezultatima ispitivanja; izdaje se za sustav 3",
        "Dokument kojim se dokazuje da proizvod zadovoljava zahtjeve stalnosti svojstava iz tehničke specifikacije; izdaje se za sustave 1+ i 1",
        "Dokument TAB-a o ocjeni svojstava; izdaje se za sve sustave"
      ],
      correct: 2,
      explanation: "Certifikat o stalnosti svojstava dokazuje da proizvod zadovoljava zahtjeve stalnosti svojstava propisane tehničkom specifikacijom; izdaje ga ovlaštena osoba na zahtjev proizvođača, ovlaštenog zastupnika ili uvoznika. Izdaje se za sustave 1+ i 1; za 2+ certifikat KTP, za 3 izvještaj o ispitivanju.",
      source: "Zakon o građevnim proizvodima; Uredba (EU) 305/2011, Prilog V",
      freq: 1
    },
    {
      q: "U oznaci betona C12/15 što znače brojevi 12 i 15?",
      options: [
        "Najmanju i najveću dopuštenu tlačnu čvrstoću u N/mm²",
        "Karakterističnu tlačnu čvrstoću na valjku 15/30 cm (12 N/mm²) i na kocki stranice 15 cm (15 N/mm²) pri starosti 28 dana",
        "Razred izloženosti i razred obradivosti",
        "Tlačnu čvrstoću pri 7 i 28 dana"
      ],
      correct: 1,
      explanation: "C12/15 je razred tlačne čvrstoće: fck,valj = 12 N/mm² (valjak 15/30 cm) odnosno fck,cube = 15 N/mm² (kocka 15 cm) pri 28 dana; najmanje 95 % rezultata mora biti ≥ propisane čvrstoće (5 % fraktil).",
      source: "HRN EN 206 — primjer izjave o svojstvima za beton"
    },
    {
      q: "Što označavaju d31.5, S2 i Cl 0,40 u oznaci betona C12/15 d31.5 S2 Cl 0,40 X0?",
      options: [
        "Debljinu zaštitnog sloja 31,5 mm, razred sulfatne otpornosti i sadržaj cementa",
        "Gustoću 31,5 kN/m³, razred smrzavanja i vodocementni faktor",
        "Maksimalno zrno agregata 31,5 mm, razred obradivosti (slijeganje) i sadržaj klorida u % mase cementa",
        "Maksimalno zrno 31,5 mm, razred čvrstoće na savijanje i količinu zraka u betonu"
      ],
      correct: 2,
      explanation: "d31.5 = maksimalno zrno agregata 31,5 mm; S2 = razred obradivosti svježeg betona mjereno slijeganjem (slump); Cl 0,40 = sadržaj klorida u % na masu cementa; X0 = razred izloženosti bez rizika od korozije.",
      source: "HRN EN 206 — primjer izjave o svojstvima za beton"
    },
    {
      q: "Koji razred izloženosti betona prema HRN EN 206 označava koroziju armature uzrokovanu kloridima iz mora?",
      options: [
        "XS (XS1–XS3)",
        "XD (XD1–XD3)",
        "XC (XC1–XC4)",
        "XA (XA1–XA3)"
      ],
      correct: 0,
      explanation: "XS — kloridi iz mora; XD — kloridi koji nisu iz mora; XC — karbonatizacija; XF — smrzavanje/odmrzavanje; XA — kemijsko djelovanje; XM — habanje; X0 — nema rizika od korozije.",
      source: "HRN EN 206"
    },
    {
      q: "Za koji od navedenih proizvoda proizvođač NIJE oslobođen obveze sastavljanja izjave o svojstvima (ZoGP čl. 27.)?",
      options: [
        "Proizvod izrađen na gradilištu pojedinačne građevine u koju će biti ugrađen",
        "Proizvod proizveden na tradicionalan način ili na način primjeren očuvanju baštine",
        "Proizvod pribavljen rušenjem građevine i pripremljen za ponovnu uporabu",
        "Serijski proizvod iz trgovine (npr. opeka) obuhvaćen usklađenom normom"
      ],
      correct: 3,
      explanation: "Iznimka vrijedi samo za proizvod izrađen prema svojstvima iz glavnog projekta ako je proizveden pojedinačno/po narudžbi u izvanserijskom postupku i ugrađen od proizvođača, izrađen na gradilištu, proizveden neindustrijski/tradicionalno/za baštinu ili pribavljen rušenjem. Serijska opeka iz trgovine mora imati izjavu o svojstvima, inače se ne smije ugraditi.",
      source: "Zakon o građevnim proizvodima, čl. 27.",
      freq: 2
    },
    {
      q: "Kako proizvođač koji je proizvod bez izjave o svojstvima (iznimka iz čl. 27. ZoGP) ujedno i ugradio preuzima odgovornost za proizvod i njegovu sigurnu ugradnju?",
      options: [
        "Izjavom upisanom u građevinski dnevnik",
        "Certifikatom odobrenog tijela",
        "Izjavom o svojstvima na hrvatskom jeziku",
        "Zapisnikom nadzornog inženjera"
      ],
      correct: 0,
      explanation: "Proizvođač koji je građevni proizvod ujedno i ugradio u tu građevinu preuzima odgovornost za proizvod i njegovu sigurnu ugradnju izjavom upisanom u građevinski dnevnik. U protivnom ga se ne smije ugrađivati.",
      source: "Zakon o građevnim proizvodima, čl. 27.",
      freq: 2
    },
    {
      q: "Gdje su u glavnom projektu navedene specifikacije proizvoda za koje proizvođač nema obvezu sastavljanja izjave o svojstvima (izrađeni na gradilištu, izvanserijski i sl.)?",
      options: [
        "U tehničkom opisu arhitektonskog projekta",
        "U troškovniku radova",
        "U programu kontrole i osiguranja kvalitete",
        "U elaboratu zaštite na radu"
      ],
      correct: 2,
      explanation: "Svojstva takvih proizvoda u odnosu na bitne značajke specificiraju se u glavnom projektu, u programu kontrole i osiguranja kvalitete.",
      source: "Zakon o građevnim proizvodima, čl. 27.; glavni projekt",
      freq: 1
    },
    {
      q: "Što je »sigurni proizvod«?",
      options: [
        "Proizvod koji ima CE oznaku i certifikat o stalnosti svojstava",
        "Proizvod koji u normalnim ili razumno predvidljivim uvjetima uporabe ne predstavlja nikakav rizik ili samo najmanji rizik spojiv s uporabom, prihvatljiv uz visoku razinu zaštite sigurnosti i zdravlja ljudi",
        "Proizvod ispitan u prijavljenom laboratoriju na reakciju na požar",
        "Proizvod koji je odobrio nadzorni inženjer"
      ],
      correct: 1,
      explanation: "Sigurni proizvod u normalnim ili razumno predvidljivim uvjetima uporabe (uključujući trajanje, stavljanje u uporabu, ugradnju i održavanje) ne predstavlja nikakav ili samo najmanji rizik spojiv s uporabom. Opasni proizvod je svaki koji nije siguran.",
      source: "Skripta — Tehnička regulativa (sigurni proizvod)",
      freq: 2
    },
    {
      q: "Što je europska tehnička ocjena (ETA)?",
      options: [
        "Certifikat prijavljenog tijela o stalnosti svojstava",
        "Usklađena europska norma bez ZA dodatka",
        "Dokumentirana ocjena svojstava proizvoda u odnosu na bitne značajke u skladu s europskim dokumentom za ocjenjivanje, koju na zahtjev proizvođača izdaje TAB",
        "Odluka Europske komisije o uzajamnom priznavanju proizvoda"
      ],
      correct: 2,
      explanation: "ETA je dokumentirana ocjena svojstava građevnog proizvoda u odnosu na bitne značajke u skladu s odgovarajućim EAD-om; izdaje ju tijelo za tehničko ocjenjivanje (TAB) na zahtjev proizvođača kada proizvod nije obuhvaćen normom.",
      source: "Uredba (EU) 305/2011, čl. 2. i 26.",
      freq: 1
    },
    {
      q: "Koji od navedenih podataka NIJE obvezni sadržaj izjave o svojstvima prema Prilogu III Uredbe 305/2011?",
      options: [
        "Jedinstvena identifikacijska oznaka vrste proizvoda i namjena",
        "Sustav ocjenjivanja i provjere stalnosti svojstava (AVCP)",
        "Usklađena norma ili EAD/ETA s prijavljenim tijelom odnosno TAB-om",
        "Cijena proizvoda i podaci o distributeru"
      ],
      correct: 3,
      explanation: "Izjava sadrži: identifikacijsku oznaku vrste proizvoda, namjenu, proizvođača, ovlaštenog predstavnika, sustav AVCP, usklađenu normu + prijavljeno tijelo (6.a) ili EAD + ETA + TAB + prijavljeno tijelo (6.b), objavljena svojstva i odgovarajuću/specifičnu tehničku dokumentaciju. Izdaje se pod isključivom odgovornošću proizvođača.",
      source: "Uredba (EU) 305/2011, Prilog III (Uredba 574/2014)",
      freq: 2
    },
    {
      q: "Što se navodi u izjavi o svojstvima ako se ona temelji na europskoj tehničkoj ocjeni (točka 6.b)?",
      options: [
        "Broj EAD-a s datumom, broj ETA s datumom, naziv TAB-a i identifikacijski broj prijavljenog tijela",
        "Samo referentni broj usklađene norme i datum izdavanja",
        "Broj hrvatske tehničke ocjene i naziv HTTO-a",
        "Broj rješenja Ministarstva o imenovanju TAB-a"
      ],
      correct: 0,
      explanation: "Ako se primjenjuje točka 6.b, navode se broj EAD-a i datum izdavanja, broj ETA i datum izdavanja, naziv tijela za tehničko ocjenjivanje i identifikacijski broj prijavljenog tijela. Kod točke 6.a navode se referentni broj usklađene norme s datumom i broj prijavljenog tijela.",
      source: "Uredba (EU) 305/2011, Prilog III"
    },
    {
      q: "Što je »specifična tehnička dokumentacija« u smislu izjave o svojstvima?",
      options: [
        "Tehnička uputa za ugradnju proizvoda na hrvatskom jeziku",
        "Dokumentacija kojom se dokazuje da su metode iz primjenjivog sustava AVCP zamijenjene drugim metodama, uz uvjet istovrijednosti rezultata s rezultatima ispitnih metoda iz usklađene norme",
        "Glavni projekt u koji se proizvod ugrađuje",
        "Zapisnik prijavljenog tijela o početnom pregledu proizvodnog pogona"
      ],
      correct: 1,
      explanation: "Specifična tehnička dokumentacija dokazuje da su metode u okviru primjenjivog sustava ocjenjivanja i provjere stalnosti svojstava zamijenjene drugim metodama, uz istovrijednost rezultata s rezultatima dobivenima ispitnim metodama iz usklađene norme.",
      source: "Uredba (EU) 305/2011, Prilog III, točka 8."
    }
  ],
  flashcards: [
    {
      front: "Što uređuje Zakon o građevnim proizvodima?",
      back: "Sustave ocjenjivanja i provjere stalnosti svojstava GP, radnje proizvođača i odobrenih tijela, dokumente ocjenjivanja, zahtjeve za prijavljena i odobrena tijela, postupak prijave i njihove obveze te druga pitanja stavljanja na tržište; uređuje primjenu Uredbe 305/2011 te usklađeno i neusklađeno područje. (ZoGP NN 76/13, 30/14, 130/17, 39/19, 118/20)",
      freq: 2
    },
    {
      front: "Usklađeno i neusklađeno područje — što je razlika?",
      back: "Usklađeno (CE): tržište EU, Uredba 305/2011 + ZoGP, usklađene tehničke specifikacije (HRN EN sa ZA dodatkom ili EAD), prijavljena tijela, TAB izdaje ETA. Neusklađeno (C): tržište RH, ZoGP Glava III., hrvatske tehničke specifikacije (tehnički propis/hrvatska norma bez ZA ili HDO), odobrena tijela, HTTO izdaje HTO. Sustavi ocjenjivanja i izjave o svojstvima su identični.",
      freq: 2
    },
    {
      front: "Što podrazumijeva usklađena (harmonizirana) tehnička specifikacija?",
      back: "1. usklađenu europsku normu (HRN EN norma koja u prilogu ima ZA dodatak); 2. europski dokument za ocjenjivanje (EAD) — kada nema primjenjive norme; na temelju njega TAB izdaje ETA.",
      freq: 2
    },
    {
      front: "Što podrazumijeva hrvatska tehnička specifikacija?",
      back: "1. tehnički propis, odnosno neusklađenu (hrvatsku) normu bez ZA dodatka na koju upućuje tehnički propis; 2. hrvatski dokument za ocjenjivanje (HDO) — na temelju kojeg HTTO izdaje hrvatsku tehničku ocjenu.",
      freq: 2
    },
    {
      front: "Što je tehnička specifikacija?",
      back: "Dokument u skladu s kojim se ocjenjuju svojstva GP u odnosu na bitne značajke; može biti usklađena/neusklađena norma ili europski/hrvatski dokument za ocjenjivanje. Sadrži minimalne zahtjeve (min. tlačna čvrstoća, otpornost na požar, vodopropusnost...). Može biti norma, dio norme ili poseban dokument.",
      freq: 2
    },
    {
      front: "Što je norma (standard)?",
      back: "Dokument koji sadrži niz preciznih definicija, kriterija, mjera i pravila koji opisuju materijale, proizvode i procese."
    },
    {
      front: "Tko donosi EAD, a tko HDO? Tko izdaje ETA, a tko HTO?",
      back: "EAD donosi tijelo za tehničko ocjenjivanje (TAB, imenovano tijelo; u RH IGH) i na temelju njega izdaje europsku tehničku ocjenu (ETA). HDO izrađuje hrvatsko tijelo za tehničko ocjenjivanje (HTTO; u RH IGH) uz prethodnu suglasnost Ministarstva i izdaje hrvatsku tehničku ocjenu (HTO).",
      freq: 1
    },
    {
      front: "Što je hrvatski dokument za ocjenjivanje (HDO) i kada se izrađuje?",
      back: "Dokument koji izrađuje HTTO u svrhu izdavanja hrvatskih tehničkih ocjena, na traženje proizvođača i uz suglasnost Ministarstva, za proizvod koji nije (u cijelosti) obuhvaćen usklađenom tehničkom specifikacijom, tehničkim propisom ili hrvatskom normom: ne potpada ni u jedno područje primjene / metoda ocjenjivanja nije prikladna za bar jednu bitnu značajku / nije određena nikakva metoda za bar jednu bitnu značajku. Sadržaj i pravila izrade propisuje ministar pravilnikom.",
      freq: 1
    },
    {
      front: "Koje dokumente izdaju prijavljena/odobrena tijela po sustavima ocjenjivanja?",
      back: "Sustav 1+ i 1: tijelo za certificiranje proizvoda → certifikat o stalnosti svojstava. Sustav 2+: tijelo za certificiranje KTP → certifikat o sukladnosti kontrole tvorničke proizvodnje. Sustav 3: laboratorij → izvještaj o ispitivanju. Sustav 4: tijelo nema zadataka.",
      freq: 1
    },
    {
      front: "Koja su prijavljena tijela, TAB, odobrena tijela i HTTO u RH?",
      back: "Prijavljena tijela: IGH, Strojarski fakultet, SŠ Bedekovčina, Geo expert, Končar, Ascon institut, Alfa atest... (lista u NANDO bazi EK). Imenovano tijelo TAB: IGH. Odobrena tijela: IGH, Strojarski fakultet, Geo expert, LTM, Zavod za ispitivanje kvalitete... HTTO: IGH."
    },
    {
      front: "Što znači »stavljanje na tržište« i »stavljanje na raspolaganje na tržištu« (Uredba 305/2011)?",
      back: "Stavljanje na tržište = prvo stavljanje na raspolaganje proizvoda na tržištu. Stavljanje na raspolaganje = svaka isporuka proizvoda za distribuiranje, potrošnju ili uporabu na tržištu u okviru trgovačke djelatnosti, s plaćanjem ili bez plaćanja.",
      freq: 1
    },
    {
      front: "Definicija građevnog proizvoda i sklopa (Uredba 305/2011)",
      back: "Građevni proizvod: svaki proizvod ili sklop proizveden i stavljen na tržište radi trajne ugradnje u građevinu ili njezine dijelove, čija svojstva imaju učinak na svojstva građevine s obzirom na temeljne zahtjeve. Sklop: GP koji je proizvođač stavio na tržište kao skupinu od najmanje dvije zasebne komponente koje se postavljaju zajedno radi ugradnje.",
      freq: 2
    },
    {
      front: "Je li oplata građevni proizvod? A skela? A prozorska kupčica?",
      back: "Ne — ne ugrađuju se trajno u građevinu i nemaju utjecaj na ispunjavanje temeljnih zahtjeva za građevinu."
    },
    {
      front: "Koje zahtjeve moraju ispunjavati građevni proizvodi prije stavljanja na tržište?",
      back: "Moraju biti sukladni zahtjevima tehničke specifikacije, imati izjavu o svojstvima bitnih značajki izdanu od proizvođača, biti označeni CE ili C oznakom i imati tehničku uputu.",
      freq: 2
    },
    {
      front: "Kako se označava građevni proizvod? Zašto od ulaska u EU nije samo CE?",
      back: "Na proizvod za koji je proizvođač sastavio izjavu o svojstvima stavlja se CE (sukladan harmoniziranoj tehničkoj specifikaciji — usklađeno područje) ili C (sukladan hrvatskoj tehničkoj specifikaciji — neusklađeno područje). Nije samo CE jer još ima proizvoda usklađenih samo s hrvatskom tehničkom dokumentacijom. Napomena: CE se izgovara »CE«, C kao slovo »C«."
    },
    {
      front: "Što izvođač utvrđuje pri preuzimanju građevnog proizvoda na gradilištu?",
      back: "1. je li proizvod namijenjen ugradnji u skladu s glavnim projektom; 2. je li isporučen s valjanom dokumentacijom (čl. 3. TPGP); 3. jesu li svojstva, bitne značajke i podaci za ugradnju sukladni glavnom projektu; 4. rok do kojega se smije ugraditi. Upisuje se u građevinski dnevnik, dokumentacija se pohranjuje na gradilištu; ako se ne ugrađuje odmah — propisno skladištiti."
    },
    {
      front: "Što sve od dokumentacije trebaju imati proizvodi koji dođu na gradilište? (čl. 3. Tehničkog propisa o građevnim proizvodima)",
      back: "Usklađeno područje: izjava o svojstvima, oznaka CE, uputa i sigurnosne obavijesti. Neusklađeno područje: izjava o svojstvima, C oznaka i tehnička uputa. Proizvod izrađen na gradilištu: izvještaj o ispitivanju odnosno drugi dokument kojim se dokazuje ispunjavanje zahtjeva iz projekta. Sve na hrvatskom jeziku latiničnim pismom.",
      freq: 2
    },
    {
      front: "Što ako proizvod na gradilištu nema navedenu dokumentaciju?",
      back: "Ne smije se ugraditi; proizvod treba vratiti proizvođaču.",
      freq: 1
    },
    {
      front: "Što ako je proizvod došao s dokumentacijom, ali nije usklađen (ima C umjesto CE)?",
      back: "Ako ima oznaku C (neusklađeno područje) može se ugrađivati u Hrvatskoj. Ako je sam proizvod drugačiji od dokumentacije (npr. druge dimenzije cigle), ne smije se ugrađivati.",
      freq: 1
    },
    {
      front: "Što ako je proizvod došao bez dokumentacije, a već smo ga počeli ugrađivati? Gdje evidentirati i koga obavijestiti?",
      back: "Uzeti uzorak, ispitati svojstva; ako ne odgovaraju — obustaviti gradnju i ukloniti nastale greške. Ugradnju/nastavak radova odobrava nadzorni inženjer. Evidentira se u građevinski dnevnik; obavještavaju se investitor, proizvođač i nadzorni inženjer.",
      freq: 1
    },
    {
      front: "Tko treba odobriti proizvod koji ima izjavu o svojstvima prije nego ga izvođač ugradi?",
      back: "Nadzorni inženjer.",
      freq: 1
    },
    {
      front: "Tehnička uporabivost građevnog proizvoda",
      back: "Proizvod je uporabljiv ako su mu tehnička svojstva sukladna tehničkoj specifikaciji (dokazuje se izjavom o svojstvima i CE ili C oznakom) i ako mu nije istekao rok upotrebe definiran u tehničkoj uputi.",
      freq: 1
    },
    {
      front: "Čime se dokazuju svojstva proizvoda izrađenog na gradilištu?",
      back: "Glavnim projektom i izvještajem o ispitivanju.",
      freq: 1
    },
    {
      front: "Razlike novog i starog Zakona o građevnim proizvodima",
      back: "Novi zakon osigurava provedbu Uredbe 305/2011: uvodi podjelu na usklađeno i neusklađeno područje, uređuje sustave ocjenjivanja i provjere stalnosti svojstava, uvodi nove pojmove (izjava o svojstvima, certifikat o stalnosti svojstava, certifikat o sukladnosti KTP, bitne značajke) i ukida sustav ocjenjivanja 2."
    },
    {
      front: "Što ako proizvod nije ni u usklađenom ni u neusklađenom području, odnosno dolazi iz druge države članice EU (ili Turske, EFTA/EGP)?",
      back: "Proizvodi na koje se ne odnosi Uredba 305/2011, a zakonito su stavljeni na tržište u drugoj članici EU, Turskoj ili EFTA državi stranci EGP-a smatraju se kompatibilnima prema Uredbi (EU) 2019/515 o uzajamnom priznavanju. Gospodarski subjekt osigurava prateću dokumentaciju na hrvatskom (namjeravana uporaba, značajke, sigurnosne obavijesti, upute o rukovanju/skladištenju/ugradnji/odlaganju); bez izjave o uzajamnom priznavanju dostavlja dokumentaciju po čl. 5. st. 5. i 6. Uredbe 2019/515.",
      freq: 1
    },
    {
      front: "Sve o Uredbi (EU) 305/2011",
      back: "Utvrđuje usklađene uvjete za stavljanje na tržište GP; stavlja izvan snage Direktivu 89/106/EEZ; na snazi od 1. 7. 2013., obvezna za sve članice. Utvrđuje: sustave AVCP, obveze gospodarskih subjekata, usklađenu tehničku specifikaciju, zahtjeve i postupke za prijavljena tijela i TAB-ove, ostala pitanja. Prilozi: I temeljni zahtjevi za građevine; II postupak donošenja EAD-a; III izjava o svojstvima; IV područja proizvoda i zahtjevi za TAB-ove; V sustavi AVCP (1+, 1, 2+, 3, 4).",
      freq: 2
    },
    {
      front: "Što mijenjaju uredbe 568/2014 i 574/2014?",
      back: "568/2014 mijenja Prilog V Uredbe 305/2011 (ocjenjivanje i provjera stalnosti svojstava). 574/2014 mijenja Prilog III (sadržaj izjave o svojstvima)."
    },
    {
      front: "C i CE oznake — tko ih stavlja, kada i kako?",
      back: "CE: jedina oznaka sukladnosti s usklađenom normom ili ETA; stavlja se samo ako je sastavljena izjava o svojstvima, vidljivo, čitljivo i neizbrisivo na proizvod ili etiketu, prije stavljanja na tržište. C: jedina oznaka sukladnosti s hrvatskom tehničkom specifikacijom; smije ju staviti samo proizvođač ili ovlašteni zastupnik, na proizvode s izjavom o svojstvima prema ZoGP-u. Šefu gradilišta znače da bez njih ne smije ugrađivati proizvod.",
      freq: 1
    },
    {
      front: "Nabroji pet sustava ocjenjivanja i provjere stalnosti svojstava i tko što radi.",
      back: "1+ : tijelo za certificiranje proizvoda — ocjenjivanje svojstava, početni pregled pogona i KTP, kontinuirani nadzor KTP, ispitivanje slučajnih uzoraka; proizvođač KTP + ispitivanje uzoraka po planu. 1: isto bez slučajnih uzoraka. 2+: proizvođač KTP, ispitivanje uzoraka i ocjenjivanje svojstava; tijelo za certificiranje KTP — početni pregled i kontinuirani nadzor. 3: proizvođač KTP; laboratorij ocjenjuje svojstva na dostavljenim uzorcima. 4: proizvođač KTP i ocjenjivanje svojstava; tijelo nema zadataka. (Prilog V Uredbe; Pravilnik NN 118/19)",
      freq: 2
    },
    {
      front: "Sustav 1+ — što je karakteristično?",
      back: "Najviše odgovornosti za prijavljeno tijelo (tijelo za certificiranje proizvoda), za razliku od sustava 4 koji sve prepušta proizvođaču. Tijelo izdaje certifikat o stalnosti svojstava na temelju ocjenjivanja svojstava (ispitivanje, proračun, tablične vrijednosti, opisna dokumentacija), početnog pregleda pogona i KTP, kontinuiranog nadzora KTP i ispitivanja slučajnih uzoraka iz pogona/skladišta. Proizvođač: KTP i ispitivanje uzoraka po planu.",
      freq: 1
    },
    {
      front: "Sustav 3 — tko je prijavljeno tijelo i što tko radi?",
      back: "Prijavljeno tijelo je ispitni laboratorij: ocjenjuje svojstva na dostavljenim uzorcima i izdaje izvještaj o ispitivanju. Proizvođač provodi samo kontrolu tvorničke proizvodnje. Primjer: krovni pokrovi koji podliježu propisima o opasnim tvarima.",
      freq: 2
    },
    {
      front: "Sustav 2+ i sustav 4 — primjeri i razlike",
      back: "2+: tijelo za certificiranje KTP provodi početni pregled i kontinuirani nadzor KTP i izdaje certifikat KTP; ocjenjivanje svojstava radi proizvođač; najsloženiji za proizvođača. 4: sve radi proizvođač (KTP + ocjenjivanje svojstava), prijavljeno tijelo nema zadataka (npr. vrata unutar zgrade). Sustav 1 primjer: proizvodi od lameliranog drva."
    },
    {
      front: "Pojmovi: gospodarski subjekt, proizvođač, distributer, uvoznik, ovlašteni predstavnik (Uredba 305/2011)",
      back: "Gospodarski subjekt = proizvođač, uvoznik, distributer ili ovlašteni predstavnik. Proizvođač: proizvodi ili je osmislio/proizveo GP i trguje njime pod svojim imenom ili žigom. Distributer: osoba u opskrbnom lancu (nije proizvođač ni uvoznik) koja stavlja GP na raspolaganje. Uvoznik: osoba s nastanom u Uniji koja GP iz treće zemlje stavlja na tržište Unije. Ovlašteni predstavnik: osoba u Uniji s pisanim ovlaštenjem proizvođača."
    },
    {
      front: "Obveze proizvođača (ZoGP čl. 30., Uredba čl. 11.)",
      back: "Sastavlja izjavu o svojstvima i stavlja CE/C; izrađuje tehničku dokumentaciju; čuva tehničku dokumentaciju i izjavu 10 godina; osigurava da serijska proizvodnja zadržava svojstva; ispituje uzorke s tržišta, evidentira pritužbe i nesukladne proizvode; osigurava broj tipa/šarže/serije te svoje ime i adresu na proizvodu; prilaže izjavu i tehničke upute na hrvatskom latiničnim pismom; kod nesukladnosti popravne mjere, povlačenje ili opoziv; kod rizika obavještava Ministarstvo i Državni inspektorat."
    },
    {
      front: "Uloge Ministarstva u provedbi Uredbe 305/2011 (ZoGP čl. 12.)",
      back: "Kontaktna točka za građevne proizvode; upravno tijelo za prijavljivanje prijavljenih tijela; upravno tijelo za imenovanje TAB-ova; član Stalnog odbora za graditeljstvo."
    },
    {
      front: "Postupak provjere ispunjavanja zahtjeva za prijavljena tijela (ZoGP čl. 12.)",
      back: "Pravna osoba podnosi zahtjev Ministarstvu; zahtjeve Uredbe dokazuje potvrdom o akreditaciji nacionalnog akreditacijskog tijela. Rješenje donosi Ministarstvo po mišljenju Povjerenstva za građevne proizvode — za laboratorij, laboratorij za požar/akustiku/opasne tvari, tijelo za certificiranje KTP, tijelo za certificiranje proizvoda. Nakon pravomoćnosti prijava EK i članicama. Privremeno rješenje ako je tijelo već prijavljeno EK, a akreditacija je u tijeku."
    },
    {
      front: "Zahtjevi za prijavljena tijela (Uredba 305/2011)",
      back: "Pravna osobnost po nacionalnom pravu; neovisna treća strana (ne smije biti projektant, proizvođač, dobavljač, instalater, kupac, vlasnik, korisnik ni održavatelj proizvoda niti njihov predstavnik; bez savjetodavnih usluga); stručno osoblje, postupci i oprema; nepristranost — naknade ne ovise o broju ni rezultatu ocjenjivanja; osiguranje od odgovornosti (osim ako odgovornost preuzme država); profesionalna tajna; sudjelovanje u koordinacijskoj skupini prijavljenih tijela."
    },
    {
      front: "Postupak imenovanja TAB-a (ZoGP čl. 16.) i postupak ukidanja ovlaštenja",
      back: "Zahtjev Ministarstvu; provjera zahtjeva iz Tablice 2 Priloga IV Uredbe; dokumentaciju propisuje ministar pravilnikom; rješenje po mišljenju Povjerenstva za jedno ili više područja proizvoda; podaci EK i članicama. Ukidanje: tijelo koje prestane ispunjavati zahtjeve obavještava Ministarstvo u roku 15 dana; provjera, mišljenje Povjerenstva o ozbiljnosti nedostataka, rok za uklanjanje ili povlačenje/ograničenje/obustava prijave odnosno imenovanja."
    },
    {
      front: "Povjerenstvo za građevne proizvode",
      back: "Obavlja poslove po ZoGP-u vezane uz stavljanje GP na tržište: savjetuje Ministarstvo kod prijavljivanja prijavljenih i odobrenih tijela te TAB-ova, kod sumnji u nedostatke i ukidanja rješenja, surađuje u nadzoru nad provedbom ZoGP-a i savjetuje glede članstva u Stalnom odboru za graditeljstvo. Članove imenuje ministar."
    },
    {
      front: "Što je certifikat o stalnosti svojstava? Za koje sustave se izdaje?",
      back: "Dokument koji dokazuje da GP zadovoljava zahtjeve stalnosti svojstava propisane tehničkom specifikacijom; izdaje ga ovlaštena osoba na zahtjev proizvođača, ovlaštenog zastupnika ili uvoznika ako su provedene propisane radnje i stalnost dokazana. Izdaje se za sustave 1+ i 1. Za 2+ certifikat o sukladnosti KTP, za 3 izvještaj o ispitivanju.",
      freq: 1
    },
    {
      front: "Objasni izjavu o svojstvima za beton C12/15 d31.5 S2 Cl 0,40 X0",
      back: "C12/15 — razred tlačne čvrstoće: fck,valj 12 N/mm² (valjak 15/30 cm), fck,cube 15 N/mm² (kocka 15 cm) pri 28 dana; ≥ 95 % rezultata iznad propisane (5 % fraktil). d31.5 — max zrno agregata 31,5 mm. S2 — razred obradivosti (slijeganje). Cl 0,40 — sadržaj klorida (% mase cementa). X0 — razred izloženosti, nema rizika od korozije. (HRN EN 206)"
    },
    {
      front: "Razredi izloženosti betona (HRN EN 206)",
      back: "X0 — nema rizika od korozije; XC — karbonatizacija (XC1–4); XF — smrzavanje/odmrzavanje sa ili bez sredstava za odleđivanje (XF1–4); XD — kloridi koji nisu iz mora (XD1–3); XS — kloridi iz mora (XS1–3); XA — kemijsko djelovanje (XA1–3); XM — habanje (XM1–3). Norma za svaki razred propisuje max v/c, min razred čvrstoće, min cementa, min zraka, agregat otporan na smrzavanje (XF), sulfatnootporni cement (XA)."
    },
    {
      front: "Za koji proizvod se ne izdaje izjava o svojstvima? (ZoGP čl. 27.)",
      back: "Iznimno, ako je proizvod izrađen prema svojstvima iz glavnog projekta i: proizveden pojedinačno ili po posebnoj narudžbi u izvanserijskom postupku i ujedno ugrađen od proizvođača; izrađen na gradilištu pojedinačne građevine; proizveden neindustrijski, tradicionalno ili za očuvanje baštine; pribavljen rušenjem građevine i pripremljen za ponovnu uporabu. Proizvođač koji ga je i ugradio preuzima odgovornost izjavom upisanom u građevinski dnevnik.",
      freq: 2
    },
    {
      front: "Gdje su u glavnom projektu navedene specifikacije proizvoda koji nemaju izjavu o svojstvima?",
      back: "U programu kontrole i osiguranja kvalitete.",
      freq: 1
    },
    {
      front: "Treba li cigla iz trgovine na gradilištu imati izjavu o svojstvima? Što ako je nema?",
      back: "Da, mora imati izjavu o svojstvima. Ako je nema, ne smije se ugraditi. Ako proizvod ne ispunjava svojstva ili je već ugrađen — proizvođač mora odmah poduzeti popravne mjere za usklađenje ili povlačenje i ponovno dokazivati svojstva."
    },
    {
      front: "Sigurni proizvod / opasni proizvod",
      back: "Sigurni proizvod: proizvod koji u normalnim ili razumno predvidljivim uvjetima uporabe (uključujući trajanje, stavljanje u uporabu, ugradnju i održavanje) ne predstavlja nikakav rizik ili samo najmanji rizik spojiv s uporabom, prihvatljiv uz visoku razinu zaštite sigurnosti i zdravlja ljudi. Opasni proizvod: svaki koji nije siguran.",
      freq: 2
    },
    {
      front: "Tehnička ocjena — hrvatska i europska",
      back: "Hrvatska tehnička ocjena: dokumentirana ocjena svojstava GP u odnosu na bitne značajke u skladu s HDO-om; izdaje ju HTTO na traženje proizvođača (uz suglasnost Ministarstva) za proizvod koji nije obuhvaćen specifikacijom/propisom/normom. Europska tehnička ocjena (ETA): dokumentirana ocjena svojstava u skladu s EAD-om; izdaje ju TAB na zahtjev proizvođača kad proizvod nije obuhvaćen normom.",
      freq: 1
    },
    {
      front: "Sadržaj izjave o svojstvima (Prilog III Uredbe 305/2011, izmjena 574/2014)",
      back: "1. jedinstvena identifikacijska oznaka vrste proizvoda; 2. namjena; 3. proizvođač; 4. ovlašteni predstavnik; 5. sustav AVCP; 6.a usklađena norma (ref. broj + datum) i prijavljeno tijelo / 6.b EAD (broj, datum), ETA (broj, datum), TAB i prijavljeno tijelo; 7. objavljena svojstva (barem jedna bitna značajka); 8. odgovarajuća i/ili specifična tehnička dokumentacija. Izdaje se pod isključivom odgovornošću proizvođača.",
      freq: 2
    },
    {
      front: "Što je specifična tehnička dokumentacija?",
      back: "Dokumentacija kojom se dokazuje da su metode u okviru primjenjivog sustava ocjenjivanja i provjere stalnosti svojstava zamijenjene drugim metodama, uz uvjet istovrijednosti rezultata s rezultatima ispitnih metoda iz usklađene norme."
    }
  ]
});
